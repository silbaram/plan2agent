...
function buildStructuredDetailProposal(run) {
  if (!['failed', 'blocked'].includes(run.status)) return null;
  const summary = structuredDetailSummary(run);
  if (!summary.missing.length) return null;
  const proposal = {
    schema_version: 'p2a.skill_proposal.v1',
    proposalId: `proposal-${safeIdPart(run.runId)}-structured-debug-detail`,
    sourceRunId: run.runId,
    problem: `Run ${run.runId} lacks structured debug detail for ${summary.missing.join(', ')}.`,
    evidence: [
      `runId: ${run.runId}`,
      `task: ${run.taskId} - ${run.taskTitle}`,
      ...structuredDetailEvidence(run),
    ],
    recommendedChange: 'Require failed/blocked run closeout to record reproduction, localization, and guard evidence before retry or maintenance triage.',
    targetFiles: ['scripts/p2a_runs.mjs', '.agents/skills/p2a-dev-execution/SKILL.md', 'docs/cli-reference.md'],
    risk: 'medium',
    status: 'proposed',
    note: 'Generated by p2a_proposals.mjs from missing structured run debug detail.',
  };
  return validateSkillProposalData(proposal);
}
...
function proposalsForRun(runsDir, run) {
  ...
  return {
    proposals: [
      ...
      buildStructuredDetailProposal(run),
...
function proposalClassification(proposal) {
  ...
  if (proposal.proposalId.includes('structured-debug-detail')) return 'structured_debug_detail';
