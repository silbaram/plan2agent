...
주의: `--task-graph`만 주고 `--require-approved-spec`를 생략하면 task graph 자체의 schema/dependency/cycle만 검증한다. Gate B 조건까지 확인하려면 `--require-approved-spec`를 함께 사용한다. `--artifact-root`는 top-level `status.md`와 gate 폴더 구조를 함께 검증한다.
...
| artifact root bundle | `--artifact-root`는 `status.md`, Gate A intake 파일, 존재하는 downstream gate의 완전한 파일 세트를 검증한다. | `Gate B is incomplete; missing ...`, `Gate C cannot be validated before Gate B spec exists` |
