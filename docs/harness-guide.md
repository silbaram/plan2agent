...
8. 모든 task를 완료하고 canonical workspace에 통합한 뒤 필요한 최종 no-change run을 연다. 승인 experience가 visual review를 요구하면 `reviewPasses.visual` 값과 무관하게 `p2a execute review`로 실제 앱 matrix와 접근성 evidence를 봉인한다. 필수 visual contract가 없는 iteration의 acceptance 기본값은 `opt_in`이며, 사용자가 독립 기능 검수를 요청해 `p2a execute accept`를 시작했거나 정책이 `on`일 때만 실제 동작 evidence를 추가로 봉인한다. 필요한 review가 끝난 뒤 현재 revision의 full test/lint/typecheck 증거가 아직 없으면 `p2a execute verify-final`에서 한 번 실행한다. Acceptance run이 같은 revision의 full evidence를 이미 기록했다면 별도 verify-final run을 만들지 않고 재사용한다. 관련 파일만 검사한 `scope: related` evidence는 이 종료 조건을 만족하지 않는다.
9. 완료 상태에서 `p2a next`는 `iteration_review_or_close_required`와 구조화된 `review`/`close` 옵션을 반환한다. 설정된 회고 임계치를 넘은 현재 iteration 신호가 있으면 v2 응답의 `retrospective`에 안전한 수치 후보를 함께 표시하고, proposal 저장 명령은 별도 승인이 필요한 review 하위 행동으로만 제공한다. `review`를 선택하면 iteration을 연 채 검토하고, finding이 있으면 옵션에 포함된 remediation command의 placeholder를 owning task id와 finding note로 바꿔 task를 reopen한다. 이후 `next`가 반환한 정상 start/finish lifecycle로 수정하고 다시 review-or-close 결정으로 돌아온다. 깨끗한 리뷰, 후보 없음, proposal 저장 거절 모두 같은 결정으로 돌아오며, 사용자가 `close`를 명시적으로 선택한 경우에만 중첩 close 명령을 실행한다.
