...
- `scripts/validate_artifacts.mjs`는 evidence `source_id` 중복과 `WEB-n` URL 형식을 검사한다. `--review`는 review artifact의 schema 유효성을 확인하고, `--require-review-pass`를 함께 쓰면 Gate D 통과 조건인 `review.blocking_issues: []`까지 확인한다. 다만 title/used_for의 내용 품질은 사람이 review해야 하므로, Gate D에서 근거가 실제 결정 근처에 인용됐는지 확인한다.
