...
8. 모든 task를 완료하고 canonical workspace에 통합한 뒤 필요한 최종 review-only run을 연다. 승인 experience가 visual review를 요구하면 `reviewPasses.visual` 값과 무관하게 `p2a execute review`로 실제 앱 matrix와 접근성 evidence를 봉인한다. 필수 visual contract가 없는 iteration의 acceptance 기본값은 `opt_in`이며, 사용자가 독립 기능 검수를 요청해 `p2a execute accept`를 시작했거나 정책이 `on`일 때만 실제 동작 evidence를 추가로 봉인한다.
9. 완료 상태에서 `p2a next`는 `iteration_review_or_close_required`와 구조화된 `review`/`close` 옵션을 반환한다. `review`를 선택하면 iteration을 연 채 검토하고, finding이 있으면 옵션에 포함된 remediation command의 placeholder를 owning task id와 finding note로 바꿔 task를 reopen한다. 이후 `next`가 반환한 정상 start/finish lifecycle로 수정하고 다시 review-or-close 결정으로 돌아온다. 깨끗한 리뷰도 같은 결정으로 돌아오며, 사용자가 `close`를 명시적으로 선택한 경우에만 중첩 close 명령을 실행한다.
