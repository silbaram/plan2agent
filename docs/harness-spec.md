...
공급자 자동 메모리는 세션 연속성을 위한 비공식 힌트다. 승인된 Gate artifact, 검사한 repository evidence 또는 commit된 BuildLore 지식을 대체하지 않는다. 서로 충돌하면 canonical artifact와 repository evidence가 우선하며, BuildLore sync와 Git publish는 계속 명시적 권한을 요구한다.
