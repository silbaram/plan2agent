...
5. `--require-monitor`로 시작한 run에만 독립 monitor 결과를 run 파일 옆의 `.monitor-verdict.json` sidecar에 기록한다. 예를 들어 run ref가 `runs/<iteration-id>/<run-id>.json`이면 verdict 경로는 `runs/<iteration-id>/<run-id>.monitor-verdict.json`이다. 이 파일은 CLI 명령으로 임의 생성하는 대신, foreground 실행 owner가 §6의 표준 JSON shape으로 작성한다. 새 `.monitor-gate.json`은 `ruleContract`에 승인 constitution 또는 legacy style의 ref와 SHA-256을 고정하고 `requiredConcernFields`에 `rule_concerns`를 포함한다. Monitor에는 이 sidecar와 규칙 원문 전체를 함께 전달한다. 일반 run은 이 단계를 건너뛴다.
   ...
   모든 task가 done이면 close-ready는 먼저 일반 구현 run과 visual/acceptance final run에서 현재 canonical revision의 configured full test/lint/typecheck 증거를 찾는다. 일반 구현 run은 isolation이 없고 완료 task에 결합되어 있으며 현재 `project.config.json`의 command identity가 정확히 일치할 때만 재사용한다. 해당 증거가 없을 때 `p2a next`가 `p2a execute verify-final --artifacts <root>`을 안내하고 canonical no-change run에서 한 번 실행한다. `related` 검증과 Gate supplemental `custom` evidence는 close-ready를 충족하지 않는다. 이후 source 또는 configured command가 변경되면 기존 증거는 stale/ineligible 처리된다.

   Child-process preflight가 `EPERM`/`EACCES` 같은 실행 환경 거부를 감지하면 product failure가 아니라 `environment_spawn_denied` unavailable evidence를 기록한다. 실패 증거를 수정하지 않고, 실행 가능한 환경에서 출력된 `p2a execute retry --artifacts <root> --run-id <run-id>` 한 명령으로 기존 final run을 `environment_failure`로 닫고 같은 task/workspace의 replacement final run을 시작한다.
...
새 run은 artifact root의 전역 `runs/run-index.json`과 iteration별 `runs/<iterationId>/<runId>.json`에 저장한다. run 파일과 index 갱신은 project run-store lock, atomic rename, 복구 journal을 사용하므로 동시 start의 lost update를 막고 중단된 commit은 다음 mutation에서 전진 복구한다. Validator는 live index entry를 canonical run 파일과 대조하고 `scope: pruned_run_history`로 구분된 retrospective의 run/status/verification/duration 합계도 검증한다. 불일치가 있으면 `p2a runs rebuild-index --runs <dir> --dry-run`으로 canonical 파일 기반 live projection을 확인한 뒤 `--yes`로 복구할 수 있으며 적용 후 검증 실패는 기존 index로 rollback한다. stale lock 회수도 별도 reaper lock으로 직렬화해 새 소유자의 lock을 이전 stale 판단으로 삭제하지 않는다. 기존 평면 `runs/<runId>.json` 이력은 계속 읽을 수 있으며, 이전 `--graph` 기본 위치였던 `iterations/<iterationId>/runs/`도 migration 입력으로 발견한다. `p2a runs migrate-layout --artifacts <artifact-root> --dry-run`으로 이동·병합 계획을 확인한 뒤 `--yes`로 정리할 수 있다. 마이그레이션은 source/target run store를 고정 순서로 잠그고 run, 알려진 sidecar, 소유 프로세스가 끝난 run ID 예약을 전역 runs로 이동한다. 살아 있는 start가 소유한 예약이 있으면 migration을 중단한다. migration journal을 확정한 즉시 legacy store에 `.run-store-redirect.json`을 남겨 이전 `--runs` 경로가 새 index를 재생성하지 못하게 한다. 프로세스가 중단되면 `.run-layout-migration` journal이 남으며 같은 `migrate-layout ... --yes` 명령으로 재개한 뒤 충돌 없는 단일 `run-index.json`을 확정하고 legacy index를 제거한다. Layout 정리와 별개로 `p2a runs migrate-schema --artifacts <artifact-root> --dry-run`은 source graph/spec provenance를 검증한 뒤 finished `p2a.run.v1` evidence에 immutable `taskContractSha256`을 계산해 v2 승격 계획을 보여준다. `--yes` 적용은 같은 run-store lock과 write journal을 사용하며 started/failed/blocked v1 이력은 자동 승격하지 않는다.
