...
허용되지 않은 verdict, verification 실패, scope drift가 있으면 run은 blocked 또는 failed 상태로 닫고, `p2a.mjs orchestrate failure-policy` 또는 `p2a.mjs proposals mine`으로 후속 조치를 만든다.
