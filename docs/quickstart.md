...
| 산출물 검증하기 | `validate_artifacts.mjs`, `p2a_iteration.mjs` | schema/gate 오류 조기 발견 |
...
| 감독형 단일 task 실행 | `p2a_execute.mjs` | task/run lifecycle 반자동 진행 |
| 개발 task 실행 관리 | `p2a_tasks.mjs` | ready/prompt/start/done 상태 전이 |
| agent 실행 결과 기록 | `p2a_runs.mjs` | `runs/run-index.json`, `runs/<runId>.json` |
| 실행 회고 개선 후보 만들기 | `p2a_proposals.mjs` | `proposals/<proposalId>.json`, `proposals/reviews/<reviewId>.json`, `proposals/curations/<curationId>.json`, `proposals/patch-drafts/<draftId>.json` |
...
`scaffold`는 co-located 정식 진입점으로, 빈 프로젝트에 하네스와 프로젝트용 `.gitignore`를 설치한다. 설치 후 `<project-dir>`에서 Claude Code/Codex/Gemini를 열고 `/p2a-harness "<한 문장 아이디어>"`를 실행한다. 산출물은 이 프로젝트의 `.plan2agent/artifacts/<project>/gate-*`에 생성된다. 승인 후 `node .plan2agent/scripts/p2a_iteration.mjs init --artifacts .plan2agent/artifacts/<project>`로 반복 구조로 전환하고, `p2a_execute.mjs`를 기본 진입점으로 개발 task 실행과 run tracking을 진행한다. `p2a_eval.mjs grade/compare/analyze`는 run 평가와 regression/failure cluster 점검에 쓰고, `p2a_tasks.mjs`/`p2a_runs.mjs`는 세부 상태 전이와 run log를 직접 다룰 때 쓴다. 설치 후 drift 점검은 `node /path/to/plan2agent/scripts/p2a_handoff.mjs update --target <project-dir>` 또는 `upgrade --dry-run`으로 파일 쓰기 없이 확인하고, 검토 후 안전 항목만 적용하려면 `--apply`를 붙인다. apply 결과는 대상 프로젝트의 `.plan2agent/update-reports/`에 남는다. `handoff`는 plan2agent에서 이미 기획한 승인 산출물을 별도 프로젝트로 옮기는 레거시/특수 흐름으로 유지된다.
...
scaffold로 하네스를 설치한 실제 프로젝트에서는 프로젝트 루트에서 `.plan2agent/scripts/` 아래의 런타임 명령만 사용한다. 개별 artifact root를 검증할 때는 다음을 사용한다.
...
```bash
node .plan2agent/scripts/p2a_iteration.mjs current \
  --artifacts .plan2agent/artifacts/<project_id>
```
...
blocked runtime에서는 `node .plan2agent/scripts/p2a_orchestrate.mjs failure-policy --runtime <runtime-path>`로 다음 조치를 `retry`, `ask_user`, `stop` 중 하나로 확인한다. 이 명령도 후속 조치만 계산하며 provider CLI나 재시도 run을 자동으로 시작하지 않는다.
...
node .plan2agent/scripts/p2a_tasks.mjs done \
  --graph .plan2agent/artifacts/<project_id>/gate-c-task-graph/task-graph.json \
  task-001
```
