...
Codex는 기본 `quality` 프로필에서 GPT-5.6 Sol과 tier별 reasoning을 사용한다. 모델 접근 권한이나 외부 provider 호환성이 필요하면 `--codex-profile inherit`으로 부모 세션 모델·reasoning을 상속할 수 있으며, 선택은 manifest에 기록되어 update/upgrade에서도 유지된다.
...
Co-located scaffold 프로젝트라면 Gate D 통과 후 먼저 `p2a.mjs iteration init`을 실행하고 `--artifacts .plan2agent/artifacts/<project_id>`를 사용한다. 아래 `--graph` 예시는 이미 승인 산출물을 별도 대상 프로젝트로 옮긴 legacy handoff 흐름에서 flat graph를 사용할 때의 형태다. legacy `--graph` 실행은 Gate B/D를 재검증하지 않는 `sourceLayout: graph`로 기록되므로 iterative milestone 완료 근거로 사용할 수 없다. `iterations/<iteration-id>/gate-c-task-graph/task-graph.json`처럼 관리형 graph를 `--graph`로 지정한 변경 명령은 거부되며 반드시 `--artifacts`를 사용해야 한다.
...
| `runs/<iterationId>/<runId>.json` | agentTool, workspaceRef, changedFiles, verification 확인 |
...
| `run-index ... does not match run file` | `runs/run-index.json`과 index의 `runRef`가 가리키는 run 파일을 수동 편집하지 않았는지 |
