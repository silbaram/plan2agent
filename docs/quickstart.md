...
| 실행 회고 개선 후보 만들기 | `p2a_proposals.mjs` | `proposals/<proposalId>.json`, `proposals/reviews/<reviewId>.json`, `proposals/curations/<curationId>.json` |
...
```text
  .plan2agent/
    artifacts/
      ...
      review.json
      status.md
    proposals/      # p2a_proposals mine/review/curate 실행 시 생성
  current-spec.json
  manifest.json
  project.config.json
scripts/
