...
| `.plan2agent/scripts/p2a_memory.mjs` | 로컬 `.plan2agent` 산출물과 Plan2Agent Memory 서버의 status/pull/search/push/digest 동기화 루프를 관리한다. |
...
4. 대상 저장소에서는 `node .plan2agent/scripts/p2a.mjs info`로 현재 상태를 확인하고, `p2a.mjs execute plan/start`로 ready task 1건의 run을 열어 감독형 agent prompt를 출력한다. 세션이 끊기면 `p2a.mjs execute resume`으로 같은 run prompt를 다시 출력한다. 복수 agent 역할이나 monitor gate가 필요한 task는 먼저 `p2a.mjs orchestrate plan`으로 실행 계획을 만든다.
...
8. 장기 보존이나 회고 검색이 필요하면 `p2a.mjs memory status/pull/push/digest`로 로컬 산출물과 Memory 서버 snapshot의 차이를 확인하고, 명시 승인 후 push한다.
...
node .plan2agent/scripts/p2a.mjs memory digest --artifacts .plan2agent/artifacts/<project_id>
...
`p2a enhance orchestration`을 적용한 프로젝트는 `.plan2agent/project.config.json.orchestration`과 `manifest.json.enhancements.orchestration`에 supervised orchestration capability가 기록된다. `p2a info`는 default mode, manifest/config sync 상태, supervised run, provider routing, monitor gate, runtime dir을 보여주고, `p2a_doctor --dev`는 orchestration manifest/config drift, runtime script, orchestration schema, supervised-only 실행 경계, provider routing, monitor gate 정책을 로컬 설정 기준으로 검사한다. 안내되는 `orchestrate plan`/`execute start`의 `--agent-tool`은 `project.config.json.devExecution`과 `manifest.json.aiToolTargets`를 기준으로 선택되며, provider asset이 없는 프로젝트는 supervised manual workflow를 안내한다.
