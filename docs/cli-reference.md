...
| `scripts/p2a_orchestrate.mjs` | ready task 1건의 supervised orchestration plan, role prompt, monitor gate, runtime sidecar를 생성·기록한다. |
...
node scripts/p2a_orchestrate.mjs runtime-status \
  --runtime .plan2agent/runs/<run-id>.orchestration-runtime.json

node scripts/p2a_orchestrate.mjs record \
  --runtime .plan2agent/runs/<run-id>.orchestration-runtime.json \
  --role implementer \
  --type status \
