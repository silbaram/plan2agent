...
| `.plan2agent/scripts/p2a_execute.mjs` | ready task 1건의 plan/start/finish/status를 묶는 단건 lifecycle primitive다. `p2a-dev-execution`은 이 primitive를 task별로 호출해 bounded ready batch를 감독한다. |
...
| `.plan2agent/scripts/p2a_memory.mjs` | 로컬 `.plan2agent` 산출물과 Plan2Agent Memory 서버의 status/push/pull/search/history/trace/impact/precedent/digest 동기화·계보 루프를 관리한다. |
...
4. 대상 저장소에서는 `node .plan2agent/scripts/p2a.mjs info`로 현재 상태를 확인하고, `p2a.mjs execute plan/start`로 ready task 1건의 run을 열어 감독형 agent prompt를 출력한다. 여러 독립 ready task를 실행할 때도 batch CLI를 만들지 않고 `p2a-dev-execution` owner가 같은 ready snapshot에서 task별 `execute start`를 직렬 호출한다. 세션이 끊기면 `p2a.mjs execute resume`으로 같은 run prompt를 다시 출력한다. 복수 agent 역할이나 monitor gate가 필요한 task는 `p2a.mjs execute start --require-monitor`로 task별 실행 계획을 만든다.
...
node scripts/p2a_handoff.mjs update --target <project-dir> [--tools all|none|codex,claude,gemini] [--codex-profile quality|inherit] [--dry-run|--apply] [--prune]
node scripts/p2a_handoff.mjs upgrade --target <project-dir> (--dry-run|--apply) [--tools all|none|codex,claude,gemini] [--codex-profile quality|inherit] [--prune]
```
...
`memory status`, `memory push`, project-scoped `memory search`, `memory history`의 일반 출력은 Memory 서버의 canonical project UUID를 `canonical project ID`로 표시한다. `memory status --output <path>`는 다음 반복이 `fresh|stale|unavailable|unchecked`를 판정할 수 있는 `p2a.memory_status.v1` 보고서를 쓴다. 모든 네트워크 요청은 `--timeout-ms` 또는 `memory.requestTimeoutMs`의 제한을 적용하며 기본값은 5000ms다. 설정된 서버가 응답하지 않으면 status 보고서를 보존하고 non-zero로 종료한다. `iteration close`는 이 read-only 점검을 임시 출력에 실행한 뒤 보고서를 교체하며, 요청별 제한과 별도로 기본 15000ms의 전체 close-time 제한을 적용한다. 점검 실패나 timeout이 발생해도 archive는 유지하고 이전 보고서를 재사용하지 않으며 동기화 미확인 경고를 출력한다. JSON 출력의 `context.projectId`는 하위호환을 위한 사람용 source key이고 `context.sourceProjectId`에도 같은 값을 명시하며, `context.canonicalProjectId`가 `/api/search/*`의 `projectId`에 전달할 authoritative UUID다. `memory search --mode semantic|hybrid`는 server-managed embedding API를 사용하고, 해당 요청이 실패하면 같은 filter의 keyword 검색을 한 번 시도한다. Global hybrid search에서 `--exclude-project` 필터가 서버의 fused candidate window를 소진해 요청 limit을 채우지 못한 경우에도 exhaustive keyword pagination으로 보완하며, 혼합 결과 여부를 `query.fallback.supplemental`에 기록한다. 성공한 fallback은 JSON의 `query.effectiveMode`와 `query.fallback`에 기록되며, keyword까지 실패하면 non-zero로 종료한다. Global search의 `--exclude-project`는 source/canonical project id로 현재 프로젝트 결과를 제거한 뒤 요청 limit을 적용한다.

`update`는 기존 scaffold 대상 프로젝트의 runtime script/schema/AI tool asset/generated file을 현재 toolkit 기준과 비교한다. 기본 실행과 `--dry-run`은 preview만 출력하고 파일을 쓰지 않는다. `upgrade --dry-run`도 같은 drift/migration 판정을 재사용한다. 출력은 `unchanged`, `missing`, `would_update`, `manual_review`, `prunable`, `retired`, `conflict`, `error`로 나뉘고, `.plan2agent/manifest.json` 또는 `project.config.json`을 읽을 수 없거나 target path가 유효하지 않으면 non-zero exit를 반환한다. manifest에서 활성화된 capability의 config가 누락되면 migration preview에 `<capability>_config: would_update`로 표시한다.
...
Scaffold가 관리하는 runtime/schema/provider 파일은 `manifest.json.managedFiles`에 소유자와 설치 당시 SHA-256이 기록된다. update/upgrade는 이전 manifest 관리 목록에서 최신 목록에 없는 경로를 이름 하드코딩 없이 폐기 후보로 찾는다. 설치 해시와 현재 파일이 같으면 `prunable`, 해시가 없거나 설치 후 수정됐으면 `retired`로 안내한다. 기본 `--apply`는 폐기 파일을 삭제하지 않으며, `--apply --prune`도 설치 해시가 현재 내용과 일치하는 일반 파일만 삭제한다. 구형 manifest의 해시 없는 파일과 사용자가 수정한 파일은 항상 남겨 두고 수동 검토 경로를 출력한다. 이미 파일이 없는 폐기 항목은 안전하게 manifest에서 정리한다.

P2A planning artifact, run log, proposal, 생성된 runtime helper의 장기 보존은 Plan2Agent Memory 동기화 또는 명시 export를 기준으로 한다. active와 closed 기능 반복의 안정 milestone review인 `midpoint.json`과 `pre_close.json`, `iteration.json`, `memory-recall*.json`은 각각 원래 반복 계보를 유지하는 Memory document snapshot 대상이며, indexed run 옆의 `<run-id>.memory-recall.json`도 해당 run의 iteration 계보를 유지한 retry recall document로 함께 push한다. 아직 검증·승격되지 않은 `*.draft.json`과 maintenance 반복은 제외한다. `memory-status.json`은 동기화 결과를 쓸 때 자기 자신을 다시 변경하는 로컬 진단 sidecar이므로 Memory snapshot 대상에서 제외한다. 같은 이유로 `iteration.json`을 snapshot으로 만들 때 `memory_freshness`만 제외하고 나머지 iteration metadata를 보존한다. `memory digest`의 usefulness 분석은 선택한 artifact root 또는 runs/eval 범위 안에서 파일명에 `memory`가 포함된 JSON 보고서만 후보로 읽으므로 unrelated project state와 update report 전체를 재귀 파싱하지 않는다. 따라서 다음 반복을 연 뒤 처음 push하더라도 이전 반복의 안정 checkpoint와 planning/retry Memory 근거가 원래 iteration 계보와 함께 보존된다. git commit은 제품 소스코드와 사람이 유지할 프로젝트 설정 이력에 집중한다.
...
1. `p2a_tasks ready` 결과를 한 ready snapshot으로 고정하고, 동일 파일·공유 설정·DB/API 계약처럼 알려진 integration surface가 겹치는 task는 제외한다.
2. committed canonical integration head를 `batchBase`로 정한다.
3. 선택한 task마다 같은 `--base-ref <batch-base>`의 fresh worktree를 만들며 `p2a_execute start`를 직렬 호출한다.
...
7. 승인된 candidate만 canonical integration branch에 반영한 뒤 task별 `p2a_execute finish`를 직렬 호출한다.
