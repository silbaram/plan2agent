...
`memory status`, `memory push`, project-scoped `memory search`, `memory history`의 일반 출력은 Memory 서버의 canonical project UUID를 `canonical project ID`로 표시한다. JSON 출력의 `context.projectId`는 하위호환을 위한 사람용 source key이고 `context.sourceProjectId`에도 같은 값을 명시하며, `context.canonicalProjectId`가 `/api/search/*`의 `projectId`에 전달할 authoritative UUID다.
