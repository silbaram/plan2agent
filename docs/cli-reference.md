...
| `scripts/p2a_proposals.mjs` | run log와 orchestration sidecar에서 Hermes식 개선 proposal 후보를 생성하고 큐를 요약한다. |
...
6. 실패, blocked monitor verdict, verification gap이 쌓이면 `p2a_proposals.mjs mine/digest`로 개선 proposal queue를 만든다. proposal 적용은 사람 승인 후 별도 변경으로만 진행한다.
...
`p2a_proposals.mjs`는 Hermes식 자가 개선 루프의 파일 기반 MVP다. run log, orchestration sidecar, monitor verdict를 읽어 skill/agent/CLI 개선 후보를 `p2a.skill_proposal.v1` JSON으로 만들고, 사람이 검토할 큐를 요약한다. proposal 적용은 자동으로 하지 않는다.
...
`digest` 결과는 개선 backlog 검토용이다. 적용은 자동으로 하지 않고, 승인된 proposal만 별도 patch 작업으로 반영한다.
