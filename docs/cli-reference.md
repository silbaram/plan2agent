...
| `.plan2agent/scripts/p2a_memory.mjs` | 로컬 `.plan2agent` 산출물과 Plan2Agent Memory 서버의 status/push/digest 동기화 루프를 관리한다. |
...
8. 장기 보존이나 회고 검색이 필요하면 `p2a.mjs memory status/push/digest`로 로컬 산출물과 Memory 서버 snapshot의 차이를 확인하고, 명시 승인 후 push한다.
