...
`push`는 같은 계획을 Memory write DTO로 변환한다. `--dry-run`은 서버에 접속하지 않고 write 순서와 수량만 보여주며 그래프 인덱스 node/edge 개수도 포함한다. 실제 서버 write는 `--yes`가 있어야 하며, project → iteration → document/proposal snapshots/chunks → task graph → tasks → runs → graph snapshot 순서로 upsert한다. task graph의 `sourceTaskGraphId`는 논리적 source identity이고, 서버가 반환한 `taskGraphId`는 dependent task write에 사용하는 canonical identity다. 클라이언트는 모든 task graph 응답의 project/iteration/source/hash가 요청과 일치하는지 먼저 검증하고, 그 뒤 각 `/api/tasks/bulk`의 `graphId`와 `task.taskGraphId`를 반환된 canonical ID로 맞춘다. 구버전 서버가 변경된 graph를 갱신하지 않아 이전 hash를 반환하면 task/run write 전에 호환성 오류로 중단하므로 서버를 먼저 업그레이드한 뒤 같은 push를 재시도한다. graph snapshot은 로컬 artifact에서 파생되는 인덱스이므로 push 때마다 선택 scope 전체를 재계산해 `/api/graph/snapshots`로 원자 교체한다. graph edge의 structured source reference는 서버의 text 계약에 맞춰 `path#fragment` 문자열로 정규화되며, node/edge id·metadata·reference 타입과 endpoint 무결성은 첫 서버 write 전에 검증된다. 구버전 Memory 서버가 graph endpoint를 404로 응답하면 graph push만 경고 후 건너뛰고 기존 artifact push는 성공시킨다. proposal snapshot은 `PROPOSAL` artifact type과 `proposalTarget`, `targetRepo`, `targetArea`, `upstreamReason` metadata를 보존한다. 인증이 필요한 Memory 서버는 `--token` 또는 `P2A_MEMORY_TOKEN` 값을 `X-P2A-Local-Token` 헤더로 받는다.
