...
5. `p2a execute status/finish`로 run 상태 확인, verification, run finish, task done/block 전이를 묶어 기록한다. Full visual iteration의 모든 구현을 통합한 뒤에는 `p2a execute review`로 이미 완료된 visual task의 canonical no-change 검토 run을 연다. 세부 제어가 필요하면 `p2a tasks`와 `p2a runs`를 직접 사용한다.
...
`--visual-experience`, `--visual-prototype`, `--visual-review`는 Gate B의 screen composition, prototype 디렉터리의 모든 regular file이 manifest 해시 집합에 포함된 passive offline HTML/CSS bundle, 실제 PNG·접근성 JSON의 해시와 capture metadata를 포함한 UI run verdict를 개별 검증한다. Full mode는 최소 두 후보를 요구하고, 각 screen state는 진입점에서 local anchor로 도달 가능한 HTML/fragment에 매핑되어야 한다. Prototype은 `script-src 'none'` CSP를 사용하며 executable JavaScript, inline event handler, 외부 navigation을 허용하지 않는다. Prototype manifest와 각 선언 파일은 해시·parse 전에 25MiB 상한을 적용하며 PNG chunk ordering, indexed palette, scanline 구조도 검증한다. `--runs-dir`는 새 `p2a.run.v2` finished run을 원본 task graph의 task와 승인 spec에 다시 결합해 run-start `taskContractSha256`, `visualReview` 계약, sidecar workspace identity/revision과 run에 봉인된 `visualReviewEvidenceSha256`이 정확히 일치하는지 확인하고, UI run에는 evidence-backed `confirm_ui` sidecar를 요구한다. Graph mode에서는 `--runs`가 원본 graph와 다른 디렉터리를 가리키거나 graph가 project-relative source spec을 참조해도 실제로 해석된 spec의 artifact root에서 provenance를 검증하며, 성공 finish 전에 같은 검사를 수행한다. `p2a runs validate --run-id`도 schema-only shortcut을 사용하지 않고 run store 전체의 index와 provenance를 함께 검증한다. 기존 `p2a.run.v1` non-visual 이력은 digest 없이도 계속 읽을 수 있으며 진행 중 v1 run은 finish 시 v2로 승격된다.
...
`start`가 출력한 prompt를 Claude Code 또는 Codex 같은 write-capable agent CLI에 붙여넣고 구현한다. Gemini CLI는 `execute review`나 monitor 같은 read-only 보조에 사용할 수 있다. `resume`은 일반 구현 run에는 launcher prompt를, `runKind: final_visual_review` run에는 동일한 capture/revision/finish 지침을 다시 출력하며 파일을 변경하지 않는다. 일반 구현 run의 `finish`는 검증 결과를 run log에 기록하고 task를 `done` 또는 `blocked`로 전이한다. `execute review`는 완료된 visual task를 다시 열지 않고 canonical workspace의 `isolation: none`, 빈 `changedFiles` run을 시작한다. 이 review가 confirm되면 task는 `done`을 유지하고, failed/blocked로 끝나면 같은 task를 사유와 함께 `todo`로 reopen하여 다음 `p2a next`가 구현 remediation을 안내한다. 이미 봉인된 final review보다 workspace가 변경된 경우 `p2a next`는 새 review를 안내한다. 봉인된 sidecar/evidence 자체가 변경되거나 누락된 경우에는 새 review로 과거 감사 손상을 가리지 않고 `p2a runs validate` 복구 경로를 안내한다. 실행 footer의 별도 proposal `review` 링크는 해당 run을 `p2a proposals mine --run-id <run-id>` 회고 후보 생성으로 연결한다.
