...
- Allocate short `iter-0001` sequence IDs for implicit iteration open and scope replacement, preserving initial and explicit IDs and historical artifact paths.
- Include supplementary report locations and task-owned temporary workspace guidance in packaged planning and execution skills.

### Fixed

- Reserve iteration sequence names from files, symlinks, and current/closed records under the existing lifecycle locks.
- Exclude untracked `.plan2agent/tmp` files from automatic change collection and managed root evidence from legacy workspace revisions, while retaining tracked and explicit changes.

## [0.5.23] - 2026-09-06

### Changed

- Exclude configured untracked generated paths from automatic change collection and documentation scans while preserving tracked, staged, explicit, and historical evidence.
- Keep shorter plain-text output tails for successful standard verification commands without rewriting prior attempts or failure history.
- Remove unused execution-envelope snapshots during whole-iteration run cleanup.

### Fixed

- Protect shared execution-envelope snapshots when unindexed recovery records remain or run-file cleanup fails.
- Apply generated-path exclusions consistently through final verification without resolving exclusion paths through external cache symlinks.
- Report all execution-envelope snapshots actually removed by run evidence garbage collection.
