...
2. Determine whether required verification was actually executed at the current revision. For each required check, use its latest decisive attempt; require `source: config` or `source: command` and `exitCode: 0`. Earlier failures remain evidence, not blockers after a valid same-run retry. Manual, unavailable, stale, or still-failing evidence is insufficient; follow the CLI's revision and verification selection rather than requiring every historical attempt to pass.
3. Determine whether `changedFiles` are inside the run `workspaceRef` and approved product scope. Installed Plan2Agent harness files are outside application implementation scope. When the approved product target is Plan2Agent itself, its canonical source files are product work and provider mirrors must come from their generator; do not block solely because an in-scope path belongs to the harness.
