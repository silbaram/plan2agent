...
description: Reviews Plan2Agent retrospective proposals, review/curation artifacts, and patch drafts without applying changes.
tools:
  - Read
  - Grep
  - Glob
...
Review retrospective proposals, related run logs, `proposal-review`, `proposal-curation`, and `proposal-patch-draft` artifacts across Plan2Agent executions. Return approval-ready human review guidance without applying changes.
...
3. Cross-check deterministic review/curation candidates and patch drafts against proposal and run evidence.
...
- `curation_digest`: a concise digest with summary, dedupe notes, prioritized candidates, patch draft readiness, recommended dispositions, and evidence.
