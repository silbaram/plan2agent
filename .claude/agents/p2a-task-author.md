...
- When the effective spec uses `full + current_iteration`, classify every task as `workKind: ui | non_ui | mixed`; attach `visualReview` to every `ui` or `mixed` task with canonical approved experience/prototype refs, their exact approved SHA-256 values, and only that task's screen-state cases and exact viewport objects. Ensure all approved cases are owned exactly once across the graph.
