...
Use the CLI result as the only decision authority. Do not infer or duplicate project-state rules.

## Procedure

For a direct request to summarize, write, or register an identified retrospective, follow the closeout reference below with the supplied report or observations. Use `next` only if completion context is missing; do not present an unrelated planning or close menu.
...
- Required, conditional; stages: closeout — `.agents/skills/p2a-dev-execution/references/verification-closeout.md` — A closeout choice or an explicit retrospective/report/issue request is being handled.
...
- `entry_deferred`: the request is saved, not permission to replace or execute current work. Wait for the user's choice; plain `next` resumes current work, and the saved entry resumes after close.
