...
6a. When the task contains `visualReview.required: true`, render the actual application for every declared `screenStates`/viewport case before finish. Use available browser tooling to capture deterministic PNG screenshots at the exact declared viewport dimensions, save them and an accessibility report under the artifact root at `visual-evidence/<iterationId>/<runId>/`, and invoke `p2a-visual-reviewer` as a separated read-only pass with the approved experience spec and selected prototype. The prototype itself is the comparison target and must never be reused as implementation evidence.

   After implementation is complete and immediately before capture, run `p2a runs revision --run-id <runId> --artifacts <artifact-root>` in the reviewed workspace. Persist the returned `p2a.visual_review.v1` object beside the run file as `<runId>.visual-review.json`. Its `workspace_ref` and `workspace_revision_sha256` must exactly match the run's `workspaceRef` and that revision command result, and its source refs must exactly match the run's copied `visualReview` contract. Every screenshot records its SHA-256, `image/png` media type, exact dimensions, capture URL, and capture timestamp. The accessibility JSON must use `p2a.visual_accessibility_report.v1`, record its tool, standard, timestamp, scanned page URLs, and violations, and its sidecar reference must include the exact SHA-256. `p2a execute finish` and `p2a runs finish` reject a successful finish unless the evidence bytes validate, the workspace revision remains unchanged, the verdict is `confirm_ui`, all declared cases pass, and accessibility passes with zero critical violations; successful finish copies the verified revision and the exact sidecar-byte `visualReviewEvidenceSha256` into the run record. Run start also records an immutable task-contract SHA-256; if the graph contract changes, finish and run-directory validation require a new run. If browser rendering is unavailable, do not infer visual correctness from source inspection; keep the run active or finish it as blocked with normal structured failure evidence.

   After all implementation changes are integrated, close-ready visual evidence requires one additional run for each visual task. Start it with `p2a execute review --artifacts <artifact-root> --task <task-id> --agent-tool <reviewer>`; this command accepts the already-`done` visual task without reopening it, records `runKind: final_visual_review`, fixes the workspace to the canonical integration workspace, uses `isolation.mode: none`, and keeps `changedFiles` empty. Capture/review the final application, then finish the run normally. If the session is interrupted, `p2a execute resume` must reprint the final-review capture/revision/finish instructions instead of an implementation prompt. Close-ready and `p2a next` compare the sealed evidence revision and digest with the current canonical workspace. A confirming review leaves the task `done`; a failed or blocked review reopens it to `todo` with a remediation note so the next action returns to implementation before another final review. A stale workspace requires a new review, while a changed or missing sealed sidecar/evidence bundle requires run-store repair through `p2a runs validate`; a new review must not conceal damaged historical evidence.
