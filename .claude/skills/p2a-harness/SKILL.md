...
3. Ask only for information or decisions that cannot be safely inferred and would materially change that scope. There is no fixed question-count or round limit on this path; stop asking as soon as the scope is confirmable. Do not materialize discovery dimensions, CQ/ND ids, canonical effects, or progress counters during this dialogue. Keep implementation-detail choices for Gate B.
