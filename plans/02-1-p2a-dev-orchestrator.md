...
작성일: 2026-06-23 · 상태: MVP 1차 구현 완료, O7 Hermes proposal queue/review MVP 완료 · 상위 문서: `plans/02-development-team-ai-agent.md` · 연결 문서: `plans/01-product-roadmap.md`, `plans/03-p2a-gui-mvp.md`
...
- `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap에서 Hermes식 proposal queue/review를 생성·검증·요약.

후속으로 남김:

- GUI 표시.
- 병렬 scheduler.
- agent curator 자동 보강과 승인 후보 diff 초안.
...
### O7. Hermes proposal queue/review MVP
...
- `list/show/digest/review/validate`로 사람이 큐와 review artifact를 검토할 수 있다.
- `validate_artifacts --proposals-dir`와 `--proposal-review`가 proposal directory와 review artifact 계약을 검증한다.
- scaffold/handoff 대상 프로젝트에 CLI와 schema가 포함된다.
- fixture가 monitor-blocked run에서 proposal을 생성하고 digest/review까지 확인한다.

후속:

- `p2a-skill-curator` agent 자동 보강 연결.
- 승인 후보 diff 초안.
...
- `p2a_proposals.mjs`가 proposal 후보와 review artifact를 생성하고 digest를 제공한다.
...
| Hermes queue/review | `p2a_proposals.mjs` deterministic mining/review + 사람 승인. 자동 적용 없음 |
