...
| Team Big Five orchestration | `p2a-dev-orchestrator` MVP 1차, runtime sidecar/communication log, supervised scheduler, GUI runtime/scheduler 표시 완료 | 감독형 multi-session PTY coordinator |
...
- v2 이후에는 Codex, Claude Code, Gemini CLI 같은 agent를 사람이 foreground에서 감독하는 orchestration을 고도화한다.

현재 기준:

- Plan2Agent는 먼저 "기획/태스크 생성 하네스"로 개발한다.
- "agent 실행 오케스트레이터"는 현재 감독형 plan/runtime sidecar MVP까지 구현했고, 구독 CLI/앱 기반 multi-session 조율은 사람이 foreground에서 승인·감독하는 coordinator로 고도화한다. API 요금제 기반 완전 자동 실행은 비용상 보류한다.
...
- PTY 기반 agent 감독 실행/감시
- Plan2Agent task를 Codex, Claude Code, Gemini CLI별 kickoff prompt로 변환하고, 사용자가 승인한 foreground 세션에 전달
- 실패 task 재시도 정책 자동화
- PR 생성 또는 변경 요약 생성
...
| Team Big Five orchestration | deterministic triage, role handoff, runtime sidecar/communication log, supervised scheduler, GUI runtime/scheduler 표시와 수동 role 상태 기록 완료 | 감독형 multi-session PTY coordinator |
...
| 우선순위 | 항목 |
| --- | --- |
| P1 | 오케스트레이션 런타임 고도화 후속: 감독형 multi-session PTY coordinator |
...
1. 오케스트레이션 런타임 고도화 후속: 감독형 multi-session PTY coordinator. 공식 CLI/앱 세션은 사용자가 foreground에서 열고 승인하며, p2a는 role/prompt/order/state만 조율한다. API 요금제 기반 완전 자동 개발은 비용상 보류하고, 구독 로그인 기반 무인 실행·browser/background loop·계정 우회는 제외한다.
