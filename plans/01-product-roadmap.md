...
이 문서는 Plan2Agent(P2A)의 제품 방향, v1 범위, 산출물 구조, task 분할 기준, 고도화 로드맵, v2 handoff, 반복/고도화 개발 구조를 정의하는 제품 정본이다. 앞으로 기능을 추가하거나 하네스를 고도화할 때 이 문서를 제품 기준으로 사용한다. 반복/고도화 개발의 상세 구현 계약은 `docs/iteration-spec.md`에서 관리한다.

## 0. 현재 완료 현황

기준일: 2026-06-24

이 문서에서 완료로 보는 범위는 "기획 산출물 생성 -> task graph 관리 -> 반복 개발 구조 -> 대상 프로젝트 handoff -> agent 실행 결과 추적 -> 감독형 오케스트레이션 계획 -> 실행 회고 proposal queue/review/curation/patch draft/approval gate -> GUI read-only 프로젝트 검사 기반과 Electron shell 1차"까지다. 실제 Codex/Claude/Gemini CLI를 Plan2Agent가 직접 PTY로 구동하고 감시하는 자동/준자동 실행기는 아직 후속이다.
...
| 실행 회고 proposal queue/review/curation/patch draft/approval execution bridge | 실패/blocked run, monitor verdict, verification gap에서 Hermes식 개선 후보, review/curation/patch-draft/approval artifact와 maintenance task를 만들고 `p2a_execute --approval`로 감독형 실행에 연결. 검색/DB 기반 Hermes 고도화는 store/DB 단계로 연기 | `p2a_proposals.mjs`, `p2a_execute --approval`, `schemas/skill-proposal.schema.json`, `schemas/proposal-review.schema.json`, `schemas/proposal-curation.schema.json`, `schemas/proposal-patch-draft.schema.json`, `schemas/proposal-draft-approval.schema.json`, `p2a-skill-curator` |
| GUI read-only shell 1차 | 선택한 프로젝트의 P2A 설치 상태, artifact layout, gate/task/run 요약, setup/import/validate 명령 안내를 파일 수정 없이 생성하고 Electron shell에서 folder picker, `--project`, 상태 화면, watcher reload로 표시 | `apps/p2a-gui/src/project-reader.mjs`, `apps/p2a-gui/bin/p2a-gui-project.mjs`, `apps/p2a-gui/src/main.mjs`, `apps/p2a-gui/src/preload.cjs`, `apps/p2a-gui/renderer/` |
...
| 개발 실행 계층 (개발팀 AI agent) | 부분 구현 — `p2a-implementer`, `p2a-performance-monitor`, `p2a-skill-curator`, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1, `p2a-dev-orchestrator` MVP 1차, Hermes proposal queue/review/curation/patch draft/approval execution bridge, GUI read-only shell 1차. 상세 `plans/02`, orchestrator 상세 `plans/02-1`, GUI 상세 `plans/03` | PTY 세션 실행, 병렬 scheduler, 자동 patch 적용은 계속 후속. Hermes 검색/DB 고도화는 store/DB 단계에서 재논의 |
| agent 자동 실행 | 실행 prompt·run log·실패 분류(C-①), Phase 1 semi-auto 실행, supervised orchestration plan/monitor gate, GUI read-only 상태 검사와 shell 표시 가능. 실행 모드는 **감독형 확정**(구독 요금제) | PTY 세션 실행 UI 미구현(상세 `plans/03`); 무인 실행·scheduler는 전용 API 키 도입 시 |
...
Plan2Agent는 사용자의 한 문장 아이디어를 출발점으로 삼아, 대화를 통해 기획을 구체화하고, 개발 가능한 명세와 task graph로 분해한 뒤, 그 task와 실행 결과를 파일 기반으로 관리하는 하네스다.

제품 방향:

- v1은 권장 모델에서 "프로젝트 scaffold -> 아이디어 입력 -> 대화 보강 -> 개발 명세 -> task graph 생성/관리 -> 실행 로그 기록 -> 다음 반복"까지 같은 코드 프로젝트 안에서 담당한다. 기존 handoff는 외부 planning bundle을 대상 프로젝트에 옮기는 레거시/특수 흐름으로 유지한다.
- v1은 실제 agent 자동 실행보다, agent가 실행할 수 있는 수준의 task와 추적 가능한 실행 기록을 만드는 데 집중한다.
- v2 이후에는 Codex, Claude Code, Gemini CLI 같은 agent를 직접 구동·감시하는 PTY orchestration을 붙인다.

현재 기준:

- Plan2Agent는 먼저 "기획/태스크 생성 하네스"로 개발한다.
- "agent 실행 오케스트레이터"는 현재 감독형 plan/sidecar MVP까지 구현했고, PTY 세션 구동·감시와 scheduler는 v2 이후 고도화 항목으로 둔다.
- 사용자가 보는 핵심 산출물은 제품 명세와 task graph다.

## 3. v1 범위

v1 범위는 아래와 같다. 각 항목의 완료 현황은 §0에서 추적하고, v1 밖 항목의 상세 계획은 §14 고도화 백로그와 연결된다.
...
- 반복 구조, semantic diff task 초안, maintenance task 관리
- 대상 프로젝트 handoff와 AI 도구/adapter 선택 설치
- task별 agent 실행 로그와 검증 결과 기록
- task 1건의 감독형 오케스트레이션 계획과 monitor gate 기록
- 실행 회고 기반 개선 proposal queue/review/curation/patch draft/approval 생성, 검증, 감독형 실행 연결

v1 밖(v2 이후 후속) 범위:

- PTY 기반 agent 세션 실행과 감시 (v2)
- PR 생성 및 리뷰 상태 연동 (v2)
- code-aware spec 역생성과 결과 diff 자동 병합 (v2)
- 병렬 실행 scheduler (v2)
- 캔버스 기반 시각 기획 입력 (v3)
- pgvector·Neo4j 기반 지식 그래프와 plan-code 계보 추적 (v3)
...
- task가 실제 agent 실행 결과와 연결되는 정보는 task graph schema를 바꾸지 않고 `runs/` sidecar에 `runId`, `changedFiles`, `verification`, `agentTool`, `workspaceRef`로 기록한다.
...
- task별 실행 로그, changed files, test/lint/typecheck 결과를 파일 기반 sidecar로 기록한다.
- branch/worktree 격리 기준을 run log에 기록하고, 명시 옵션을 줄 때만 선택 생성한다.

현재 완료된 handoff/run tracking 범위:
...
- 실행 로그 저장
- worktree 또는 branch 격리 기준 기록과 선택 생성
- 결과 diff와 task 연결

후속 범위:

- task별 agent 세션 생성과 PTY 기반 감시
- 실패/재시도/중단 상태 자동화
- PR 생성과 리뷰 상태 연동
- 병렬 agent 실행 scheduler
...
- v1은 agent CLI를 직접 PTY로 구동하지 않는다. 대신 실행 기록, 감독형 단일 task 절차, 오케스트레이션 계획은 파일 기반 CLI로 제공한다.
- 개발 대상 프로젝트에 산출물과 AI 개발 도구를 인계하는 부트스트랩 도구는 완료된 기준으로 둔다.
- agent 실행 결과는 파일 기반 run log와 orchestration sidecar로 기록하고, 실제 세션 실행/감시는 후속 PTY 실행기로 분리한다.
- agent 세션 실행의 첫 연동 대상은 Codex로 둔다.

## 8-1. v2 개발 인계와 환경 세팅

이 축의 1차 목표는 v1 하네스가 만든 산출물을 실제 개발 프로젝트로 옮기고, 그 프로젝트 안에서 AI agent가 바로 task를 실행할 수 있도록 개발 도구를 설치하는 것이다. 현재는 co-located scaffold와 legacy handoff 모두 완료된 상태이며, agent CLI 직접 PTY 실행과 병렬 scheduler는 후속이다.

현재 구현 상태:

- 완료: Gate D 통과 여부 확인, 원본 `artifacts/<project_id>/` 검증, 산출물 copy/move, `--include-intake`, `--overwrite`, `--dry-run`, `.plan2agent/artifacts/` 생성, `manifest.json`, `project.config.json`, `p2a_tasks.mjs`, `p2a_runs.mjs`, `p2a_execute.mjs`, `p2a_orchestrate.mjs`, `p2a_proposals.mjs`, `validate_artifacts.mjs`, schema 복사.
...
- 제외: AI agent 직접 PTY 실행, 코드 변경 자동 생성, PR 생성, 여러 agent 병렬 실행 조율, 무인 scheduler.

핵심 흐름:

1. Gate D를 통과한 `artifacts/<project_id>/` 산출물을 선택한다.
2. 사용자가 지정한 개발 대상 디렉터리로 산출물을 복사하거나 이동한다.
3. 대상 프로젝트 안에 `.plan2agent/` 작업 디렉터리를 만든다.
...
8. 필요하면 `p2a_orchestrate.mjs`로 감독형 실행 계획을 만들고, `p2a_execute.mjs`로 run lifecycle을 기록한다.
...
- 승인된 spec이 라이브러리, 프레임워크, 런타임, 프로토콜, 데이터베이스, 클라우드 서비스, 외부 API 같은 material technology choice를 담으면 Gate B Technology Reconnaissance의 `WEB-n` 근거가 최소 하나 있다.
  ...
  "externalHarnesses": [
    {
      "name": "team-bigfive",
      "sourceType": "local",
      "source": "/path/to/team-bigfive",
      "sourceVersion": "1.2.3",
      "targets": ["codex", "claude", "gemini"],
      "sourceManifest": ".plan2agent/team-harnesses/team-bigfive/source-manifest.json",
      "adaptationNotes": ".plan2agent/team-harnesses/team-bigfive/adaptation-notes.md",
      "adapterFiles": []
    }
  ],
  "artifactFiles": [
...
- handoff는 네트워크 clone이나 agent 실행을 하지 않는다. `--team-bigfive-source`가 Git URL이면 URL provenance만 기록하고, local directory이면 파일 목록과 SHA-256을 `source-manifest.json`에 기록한다.
- local source이고 Claude target이 포함되면 안전 필터를 통과한 원본 파일을 `.claude-plugin/team-bigfive/source/`에 복사한다.
...
7. 실패/blocked run이나 verification gap이 쌓이면 `p2a_proposals.mjs mine/review/curate/draft-patch/approve-draft/digest`로 개선 후보 큐와 review/curation/patch-draft/approval artifact를 만들고, 승인된 항목은 `p2a_execute --approval`로 maintenance task 실행에 연결한다.
...
- 대상 프로젝트에 `p2a_tasks.mjs`, `p2a_runs.mjs`, `p2a_execute.mjs`, `p2a_orchestrate.mjs`, `p2a_proposals.mjs`, `validate_artifacts.mjs`, schema가 복사된다.
...
- 완료: Gate B 승인 시 `current-spec.json.gate_b_approval_audits`와 `status.md`의 Gate B approval audit block을 기록·검증한다(`init`, `promote-spec`, `validate --stage gate-b-approved`).
- 완료: 승인 spec에 material technology choice가 있으면 Gate B Technology Reconnaissance `WEB-n` 근거를 요구하고, delta draft와 `current-spec.json` 조합 시 baseline `WEB-n` 근거를 이월한다.
- 완료: `status.md`가 Progress line과 1~5 섹션, Gate A-D 구조를 갖추도록 `validate`/`validate --allow-planning`에서 검증한다.
- 완료: `p2a_iteration.mjs diff-tasks`가 active spec과 baseline field 차이를 semantic group으로 병합/분할해 Gate C task graph 초안을 생성한다.
...
- 완료: `p2a_iteration.mjs validate`가 기본으로 close 시점 존재 여부/hash를 감사해 archived artifact 변경을 감지한다. legacy/migration 우회는 `--skip-archive-audit`로 한다.
- 완료: `p2a_iteration.mjs maintenance add`가 상시 maintenance task graph를 lazy 생성/append한다.
- 완료: `status.md`가 전체 반복 history, close audit, handoff audit, maintenance 요약을 누적 렌더링한다.
- 완료: 반복 handoff가 handoff 기준점을 `current-spec.json.handoff_records`/`status.md`에 기록하고, maintenance graph가 있으면 `.plan2agent/maintenance/task-graph.json`으로 별도 복사한다.
- 완료: `promote-tasks`가 agent-authored Gate C 초안의 provenance sidecar(`task-graph.draft.meta.json`)를 기록한다.
- 부분 완료: 구조적 diff와 사용자 질문 재생성을 포함한 baseline-aware intake/spec 고도화.
- 완료: 구조적 diff 기반 재작업 task 자동 생성. `diff-tasks`가 semantic group, 완료 task overlap 기반 rework, `--force` 미완료 task reuse, verification dependency, question disposition 재처분 acceptance를 기록한다.
- 완료: agent 실행 로그, branch/worktree 격리 기준, changedFiles, test/lint/typecheck 결과를 `p2a_runs.mjs` run log로 기록한다.
...
| 5-2 | diff 기반 task graph 초안 | 완료 | `diff-tasks`가 field diff를 semantic group으로 병합/분할하고 rework/reuse/verification dependency를 포함한 task graph 초안을 만든다. |
...
- task별 `runId`, `changedFiles`, `verification`, `startedAt`, `finishedAt`, `agentTool`, `workspaceRef` 기록
- `--isolation branch|worktree` 격리 기준 기록과 `--create-isolation` 선택 생성
- `--collect-git` 기반 변경 파일 수집
- `.plan2agent/project.config.json` 또는 명시 옵션 기반 test, lint, typecheck 결과 수집
- handoff 대상 프로젝트에 `p2a_runs.mjs`, run schema, `project.config.json.runTracking` 설치
- `validate_artifacts.mjs --run`, `--run-index`, `--runs-dir` 검증

후속 범위:

- PTY 기반 agent 자동 실행/감시
- Plan2Agent task를 Codex, Claude Code, Gemini CLI별 kickoff prompt로 자동 변환해 실행
- 실패 task 재시도 정책 자동화
- PR 생성 또는 변경 요약 생성

run log는 task graph schema를 바꾸지 않고 task와 실제 코드 변경의 계보를 연결하는 sidecar다.
...
- 결정됨: `runs/` 실행 로그 디렉터리는 `p2a_runs.mjs`가 관리한다.
...
- v1의 핵심 기획 산출물은 task graph다.
- 개발 handoff 이후 실행 추적은 task graph를 바꾸지 않고 run log sidecar로 관리한다.
...
| Hermes식 proposal queue/review/curation/patch draft/approval execution bridge | 완료 | `p2a_proposals.mjs`가 proposal/review/curation/patch-draft/approval JSON을 생성하고, `p2a_execute --approval`이 승인 항목을 maintenance task 실행으로 연결한다. |
...
| 단계 | 항목 |
| --- | --- |
| v2 | 개발 실행 계층 구성 **(부분 구현)**: implementer/monitor/curator, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1, `p2a-dev-orchestrator` MVP 1차, Hermes proposal queue/review/curation/patch draft/approval execution bridge, GUI read-only shell 1차 완료. 남은: PTY 세션 실행, GUI artifact viewer/local config hardening, scheduler, 자동 patch 적용. planning harness는 read-only 유지. 상세 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md`, `plans/03-p2a-gui-mvp.md` |
| v2 | 감독형 agent 실행기(Phase 1 semi-auto + orchestration plan + GUI read-only shell 1차 완료 → Phase 2 PTY 감독 실행 화면). 무인(headless) 실행·세션 감시는 전용 API 키 도입 시 (LD-9, `plans/02`) |
...
10. 개발 실행 계층(개발팀 AI agent)을 부분 구현했다: `p2a-implementer`(Codex workspace-write·Claude deny+hook+macOS sandbox confinement), 독립 `p2a-performance-monitor`, `p2a-skill-curator`+proposal schema, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1(`p2a_execute.mjs`), 감독형 오케스트레이션 계획 MVP(`p2a_orchestrate.mjs`, run sidecar, monitor gate), Hermes proposal queue/review/curation/patch draft/approval execution bridge(`p2a_proposals.mjs`, `p2a_execute --approval`), GUI read-only shell 1차(`apps/p2a-gui`). 실행 모드는 구독 요금제로 인해 **감독형**으로 확정(무인은 전용 API 키 시). Gemini는 read-only 고정. 상세·결정은 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md`, `plans/03-p2a-gui-mvp.md`.
...
1. 개발 실행 계층 **이어서 구현**: implementer/monitor/curator/C-①, **감독형 단일 task 실행기 Phase 1(`p2a_execute.mjs`)**, **조율 `p2a-dev-orchestrator` MVP 1차**, **Hermes proposal queue/review/curation/patch draft/approval execution bridge(`p2a_proposals.mjs`, `p2a_execute --approval`)**, **GUI read-only shell 1차(`apps/p2a-gui`)**은 완료. 다음은 PTY 감독 실행 화면, GUI hardening, 또는 scheduler다. planning harness는 read-only 유지. 상세 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md`, `plans/03-p2a-gui-mvp.md`.
