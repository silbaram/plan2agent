...
| 감독형 오케스트레이션 MVP | ready task 1건을 `solo`/`solo_monitor`/`team`으로 triage하고 role prompt, run/runtime sidecar, monitor gate 연결 |
...
| Team Big Five orchestration | `p2a-dev-orchestrator` MVP 1차와 runtime sidecar/communication log 완료 | 팀 세션 scheduler, 자동 role/monitor 호출 |
...
- v2 이후에는 Codex, Claude Code, Gemini CLI 같은 agent를 직접 구동·감시하는 orchestration을 고도화한다.

현재 기준:

- Plan2Agent는 먼저 "기획/태스크 생성 하네스"로 개발한다.
- "agent 실행 오케스트레이터"는 현재 감독형 plan/runtime sidecar MVP까지 구현했고, multi-session scheduler와 완전 자동 실행은 후속 고도화로 둔다.
...
- agent 실행 결과는 파일 기반 run log로 기록하고, 자동 실행은 후속 실행기로 분리한다.
...
| Team Big Five orchestration | deterministic triage, role handoff, runtime sidecar/communication log 완료 | 실제 team session scheduler와 자동 role/monitor 호출 |
...
| 우선순위 | 항목 |
| --- | --- |
| P1 | 오케스트레이션 런타임 고도화 2단계: team session scheduler와 자동 role/monitor 호출 |
| P1 | orchestrator와 최신 GUI 실행 화면 연결: mode/role handoff/monitor gate 표시 |
...
1. 오케스트레이션 런타임 고도화 2단계: team session scheduler와 자동 role/monitor 호출. 상세는 `plans/02-1-p2a-dev-orchestrator.md`.
2. 최신 GUI에 orchestration mode, role handoff, monitor gate 상태 표시.
3. Hermes cross-session recall은 Task Store/DB 설계 시점에 재논의.
4. PR 생성 및 리뷰 상태 연동.
