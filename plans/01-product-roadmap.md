...
이 문서에서 완료로 보는 범위는 "기획 산출물 생성 -> task graph 관리 -> 반복 개발 구조 -> 대상 프로젝트 handoff -> agent 실행 결과 추적 -> 감독형 오케스트레이션 계획 -> 실행 회고 proposal queue"까지다. 실제 Codex/Claude/Gemini CLI를 Plan2Agent가 직접 PTY로 구동하고 감시하는 자동/준자동 실행기는 아직 후속이다.
...
| 실행 회고 proposal queue | 실패/blocked run, monitor verdict, verification gap에서 Hermes식 개선 후보를 생성·검증·요약 | `p2a_proposals.mjs`, `schemas/skill-proposal.schema.json`, `p2a-skill-curator` |
...
| 개발 실행 계층 (개발팀 AI agent) | 부분 구현 — `p2a-implementer`, `p2a-performance-monitor`, `p2a-skill-curator`, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1, `p2a-dev-orchestrator` MVP 1차, Hermes proposal queue MVP. 상세 `plans/02`, orchestrator 상세 `plans/02-1` | curator review 자동화, PTY+Electron 감독 GUI 표시/세션 실행, 병렬 scheduler |
...
- 실행 회고 기반 개선 proposal queue 생성과 검증
...
7. 실패/blocked run이나 verification gap이 쌓이면 `p2a_proposals.mjs mine/digest`로 개선 후보 큐를 만든다.
...
| Hermes식 proposal queue MVP | 완료 | `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap에서 `skill-proposal` JSON을 생성하고 digest/validate 흐름을 제공한다. |
...
| 단계 | 항목 |
| --- | --- |
| v2 | 개발 실행 계층 구성 **(부분 구현)**: implementer/monitor/curator, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1, `p2a-dev-orchestrator` MVP 1차, Hermes proposal queue MVP 완료. 남은: curator review 자동화, PTY+Electron 감독 GUI, scheduler. planning harness는 read-only 유지. 상세 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md` |
...
10. 개발 실행 계층(개발팀 AI agent)을 부분 구현했다: `p2a-implementer`(Codex workspace-write·Claude deny+hook+macOS sandbox confinement), 독립 `p2a-performance-monitor`, `p2a-skill-curator`+proposal schema, 실패 분류(C-①), 자가발전 1사이클, 감독형 단일 task 실행기 Phase 1(`p2a_execute.mjs`), 감독형 오케스트레이션 계획 MVP(`p2a_orchestrate.mjs`, run sidecar, monitor gate), Hermes proposal queue MVP(`p2a_proposals.mjs`). 실행 모드는 구독 요금제로 인해 **감독형**으로 확정(무인은 전용 API 키 시). Gemini는 read-only 고정. 상세·결정은 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md`.
...
1. 개발 실행 계층 **이어서 구현**: implementer/monitor/curator/C-①, **감독형 단일 task 실행기 Phase 1(`p2a_execute.mjs`)**, **조율 `p2a-dev-orchestrator` MVP 1차**, **Hermes proposal queue MVP(`p2a_proposals.mjs`)**는 완료. 다음은 curator review 자동화 또는 PTY+Electron 감독 GUI다. planning harness는 read-only 유지. 상세 `plans/02-development-team-ai-agent.md`, `plans/02-1-p2a-dev-orchestrator.md`.
