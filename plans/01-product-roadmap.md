...
| GUI/design-system 제거 | 2026-07-14 결정: 도그푸딩(webhook-relay, run 51건) 전 구간 GUI 사용 0회였고 실제 감독 표면은 agent 대화 세션이며 대화 질의가 진행 상태 파악에 더 정확했다. GUI는 스키마/CLI 계약의 두 번째 소비자로 유지보수 비용만 만들었으므로 `apps/p2a-gui`와 유일 소비자인 `p2a-design-system` skill, Gemini command, Astryx 파일럿을 함께 제거한다. 코드는 git 이력으로 보존하고 아카이브 디렉터리는 만들지 않으며, 비대화형 provider 감독이나 병렬 run 관제가 실수요로 발생할 때만 재검토한다. |
...
| P2 | maintenance UX | graph 생성/검증/handoff 가능 | maintenance 전용 draft/승격 흐름 정리 |
...
2. Memory 서버와 동기화하는 `status/push` 클라이언트 계약을 설계한다.
3. run/proposal 기록을 기반으로 `p2a_eval analyze/compare` 초안을 만든다.
4. 여러 프로젝트에 설치된 P2A 하네스를 관리하기 위해 `upgrade --dry-run`과 drift report를 정리한다.
