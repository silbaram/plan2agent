...
- `p2a_orchestrate next-role/role-prompt/mark-role/failure-policy`: runtime 상태에서 다음 role을 계산하고, 사람이 공식 CLI/앱에 붙여넣을 prompt를 출력하고, 사람이 수행한 role 상태 전이를 기록하며, blocked/failed runtime의 다음 조치를 `retry`/`ask_user`/`stop` 중 하나로 계산한다. 이 명령들은 Codex/Claude/Gemini 프로세스를 자동 실행하지 않는다.
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 + runtime 1단계 + supervised scheduler 2단계 + failure-policy + provider-native runner guide/doctor 완료)
   ...
   - 후속 범위: agent-generated orchestration plan. 구독 로그인 기반 무인 실행, 계정 내부 자동 introspection, 자동 role/monitor 호출은 계속 제외한다.
