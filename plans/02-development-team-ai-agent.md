...
작성일: 2026-06-16 · 갱신일: 2026-06-24 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차 완료, 실행기는 감독형 방향(구독 로그인 — 무인은 API 키 도입 시 보류)
...
- run sidecar/failureClass/monitor verdict를 모아 proposal backlog로 만드는 Hermes queue.
...
4. **Hermes식 proposal/curator 루프 고도화** (다음 추천)
   - 목적: 실제 run에서 쌓인 proposal을 normalize, dedupe, prioritize하고 승인 대기 목록으로 정리한다.
   - 범위: proposal 자동 적용은 계속 금지한다.
   - 산출물 후보: run sidecar/failureClass/monitor verdict 기반 proposal queue, review digest, 승인 후보 diff 초안, 반복적으로 발생한 실패 유형 통계.

5. **감독형 실행기 Phase 2: PTY+Electron GUI**
