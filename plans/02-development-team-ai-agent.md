...
작성일: 2026-06-16 · 갱신일: 2026-06-24 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차 완료, Hermes proposal queue/review MVP 완료, 실행기는 감독형 방향(구독 로그인 — 무인은 API 키 도입 시 보류)
...
| Skill evolution | proposal queue/review MVP 완료 | `p2a-skill-curator`, `schemas/skill-proposal.schema.json`, `schemas/proposal-review.schema.json`, `p2a_proposals.mjs` |
...
- `p2a_proposals.mjs`: run log, orchestration sidecar, monitor verdict에서 proposal 후보를 생성하고 list/show/validate/digest/review를 제공한다.
...
- proposal review를 agent curator가 자동 보강하고 승인 후보 diff 초안까지 만드는 흐름.
...
| Hermes proposal queue/review MVP | 완료 | `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap에서 proposal JSON과 review artifact를 생성·검증·요약 |
...
| LD-5 | 자가 발전 | JSON proposal queue/review + read-only curator + 사람 승인 | `skill-proposal.schema.json`, `proposal-review.schema.json`, `p2a_proposals.mjs`, `p2a-skill-curator`가 존재한다 |
...
| retrospective proposal | `schemas/skill-proposal.schema.json`, `schemas/proposal-review.schema.json`, `.agents/agents/p2a-skill-curator.md`, `p2a_proposals.mjs` |
...
| Hermes proposal queue/review | `p2a_proposals.mjs`, `validate_artifacts --proposals-dir`, `validate_artifacts --proposal-review`, scaffold/handoff manifest |
...
| Skill evolution | 실행 회고를 proposal/review로 축적 | read-only + proposal review | `p2a-skill-curator`, `skill-proposal`, `proposal-review` |
...
- `p2a_proposals.mjs`가 `failed`/`blocked` run, monitor verdict, verification gap을 읽어 proposal queue와 review artifact를 만든다.
...
4. **Hermes식 proposal queue/review MVP** (✅ 완료)
   - 목적: 실제 run에서 쌓인 proposal을 normalize, dedupe, prioritize하고 승인 대기 목록으로 정리한다.
   - 범위: proposal 자동 적용은 계속 금지한다.
   - 완료 범위: run sidecar/failureClass/monitor verdict 기반 proposal queue, invalid run best-effort skip, list/show/validate/digest/review, proposal-review schema, scaffold/handoff 복사, fixture 회귀.
   - 후속 범위: agent curator 자동 보강, 승인 후보 diff 초안, skipped-verification rationale 표준 필드/marker, 반복 failureClass/source/targetFiles 통계 고도화.
...
| 자가 발전 (proposal→curator→승인→적용) | ✅ | 첫 사이클 완결 + `p2a_proposals.mjs` queue/review MVP. 자동 적용은 금지 |
| staged proposal→review→approval→apply | ✅ | `skill-proposal.schema.json` + `proposal-review.schema.json` + `p2a_proposals.mjs` + `p2a-skill-curator` |
...
- ✅ **Hermes proposal queue/review MVP** — 완료. `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap을 proposal JSON으로 축적하고 deterministic review/digest/validate를 제공한다. 자동 적용은 계속 금지.
