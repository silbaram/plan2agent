...
작성일: 2026-06-16 · 갱신일: 2026-06-24 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차 완료, Hermes proposal queue/review/curation/patch draft MVP 완료, 실행기는 감독형 방향(구독 로그인 — 무인은 API 키 도입 시 보류)
...
현재 저장소는 "기획 산출물 생성 -> task graph 관리 -> 반복 구조 -> handoff/scaffold -> run log 기록 -> 감독형 단일 task 실행 절차 -> 감독형 오케스트레이션 계획 -> 실행 회고 proposal queue/review/curation/patch draft"까지 구현되어 있다.

자동 PTY 실행기나 PR 생성기는 아직 없다. 대신 **Codex(native sandbox)·Claude(deterministic confinement) write-capable implementer**와, **CLI 공통으로 동작하는 독립 monitor·skill curator(둘 다 read-only)·retrospective proposal schema/queue/review/curation/patch draft**, ready task 1건의 `plan/start/finish/status` lifecycle을 묶는 `p2a_execute.mjs` Phase 1 실행기, `solo`/`solo_monitor`/`team` 실행 계획을 만드는 `p2a_orchestrate.mjs` MVP, run failure/monitor verdict/verification gap을 proposal로 남기고 deterministic review/curation/patch-draft artifact를 만드는 `p2a_proposals.mjs` MVP까지 들어와 있다. Claude confinement은 scaffold가 설치하는 공통 deny-rules + cross-platform PreToolUse hook(symlink/junction 우회까지 canonicalize로 차단, fail-closed)에 macOS/Linux는 OS sandbox 토글을 더한 형태다. 무인 자율 모드(permissionMode) 전환 자체는 실기 cross-OS spike 후 사람이 결정하며, 그 전까지 implementer는 foreground로 둔다. Gemini implementer는 설계상 read-only로 고정한다(자율/write 미추진 결정, LD-8). Phase 2 PTY+Electron GUI MVP 범위는 `plans/03-p2a-gui-mvp.md`에서 관리한다.
...
| Supervised orchestration planner | MVP 1차 완료 | `p2a_orchestrate.mjs`, `orchestration-plan.schema.json`, `p2a_execute --orchestration-plan`, `runs/<runId>.orchestration.json` |
...
| Skill evolution | proposal queue/review/curation/patch draft MVP 완료 | `p2a-skill-curator`, `schemas/skill-proposal.schema.json`, `schemas/proposal-review.schema.json`, `schemas/proposal-curation.schema.json`, `schemas/proposal-patch-draft.schema.json`, `p2a_proposals.mjs` |
| Agent 자동 실행 | 부분 구현 | Phase 1 CLI semi-auto와 supervised orchestration plan 완료. PTY/session 실행, scheduler, retry, PR 연동은 후속 |
...
- **개발팀 컨셉 - Team Big Five**: 복잡한 task를 `team-lead`/`contributor`/`performance-monitor` 역할로 협업·교차검증한다. 현재 P2A에는 Team Big Five adapter 설치, `p2a-performance-monitor`, `p2a-dev-orchestrator` MVP 1차, deterministic solo/team triage가 들어와 있다. PTY 팀 실행기, shared mental model, closed-loop communication은 후속이다.
...
- `p2a_execute --orchestration-plan`: plan을 `runs/<runId>.orchestration.json` sidecar로 연결하고, monitor verdict가 필요한 run은 finish 전에 verdict를 확인한다.
...
- `p2a_proposals.mjs`: run log, orchestration sidecar, monitor verdict에서 proposal 후보를 생성하고 list/show/validate/digest/review/curate/draft-patch를 제공한다.
...
| supervised orchestration plan MVP | 완료 | `p2a_orchestrate.mjs`, run sidecar, monitor gate fixture |
| Hermes proposal queue/review/curation/patch draft MVP | 완료 | `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap에서 proposal JSON, review artifact, curation artifact, patch draft를 생성·검증·요약 |
...
| LD-4 | 실행 모드 | `solo`/`solo_monitor`/`team` deterministic plan + monitor gate MVP. 실제 multi-session PTY scheduler는 후속 | `p2a_orchestrate.mjs`와 `p2a_execute --orchestration-plan`이 plan/sidecar 계약을 제공한다 |
| LD-5 | 자가 발전 | JSON proposal queue/review/curation/patch draft + read-only curator + 사람 승인 | `skill-proposal.schema.json`, `proposal-review.schema.json`, `proposal-curation.schema.json`, `proposal-patch-draft.schema.json`, `p2a_proposals.mjs`, `p2a-skill-curator`가 존재한다 |
...
| retrospective proposal | `schemas/skill-proposal.schema.json`, `schemas/proposal-review.schema.json`, `schemas/proposal-curation.schema.json`, `schemas/proposal-patch-draft.schema.json`, `.agents/agents/p2a-skill-curator.md`, `p2a_proposals.mjs` |
...
| supervised orchestration plan | `p2a_orchestrate.mjs`, `schemas/orchestration-plan.schema.json`, `p2a-dev-orchestrator`, run sidecar |
| Hermes proposal queue/review/curation/patch draft | `p2a_proposals.mjs`, `validate_artifacts --proposals-dir`, `validate_artifacts --proposal-review`, `validate_artifacts --proposal-curation`, `validate_artifacts --proposal-patch-draft`, scaffold/handoff manifest |

후속 out:

| 항목 | 미루는 이유 |
| --- | --- |
| PTY/session runtime | 현재는 CLI-first foreground 감독형 실행이 정본이고, 실제 세션 구동/감시는 Phase 2 GUI에서 다룬다 |
...
| 병렬 scheduler | 여러 세션을 실제로 자동 조율하기 전 run/monitor/proposal 데이터가 더 필요하다 |
...
| Skill evolution | 실행 회고를 proposal/review/curation/patch draft로 축적 | read-only + proposal review/curation/patch draft | `p2a-skill-curator`, `skill-proposal`, `proposal-review`, `proposal-curation`, `proposal-patch-draft` |
...
- **현재 기본 = supervised single task + 선택적 orchestration plan/monitor**: `p2a_execute.mjs`가 ready 확인, run start, manual launcher prompt, verify, finish, task done/block 전이를 묶고, `p2a_orchestrate.mjs`는 필요한 경우 역할별 실행 계획과 monitor gate를 sidecar로 연결한다. `p2a-dev-execution`은 사람이 감독하는 구현 절차를 안내한다.
...
- `p2a_proposals.mjs`가 `failed`/`blocked` run, monitor verdict, verification gap을 읽어 proposal queue와 review/curation/patch-draft artifact를 만든다.
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 완료)
   - 목적: Team Big Five의 team-lead 역할을 P2A-native로 분리한다.
   - 완료 범위: ready task 1건을 `solo`/`solo_monitor`/`team`으로 deterministic triage하고, 역할별 prompt, run sidecar, monitor gate를 남긴다.
   - 후속 범위: shared mental model, closed-loop communication, 실제 multi-session PTY scheduler.

4. **Hermes식 proposal queue/review/curation/patch draft MVP** (✅ 완료)
   - 목적: 실제 run에서 쌓인 proposal을 normalize, dedupe, prioritize하고 승인 대기 목록으로 정리한다.
   - 범위: proposal 자동 적용은 계속 금지한다.
   - 완료 범위: run sidecar/failureClass/monitor verdict 기반 proposal queue, invalid run best-effort skip, list/show/validate/digest/review/curate/draft-patch, proposal-review/curation/patch-draft schema, scaffold/handoff 복사, fixture 회귀.
...
| team-lead / orchestrator | 🟡 | `p2a-dev-orchestrator` MVP 1차 완료. 실제 multi-session 조율은 후속 |
...
| 자가 발전 (proposal→curator→승인→적용) | ✅ | 첫 사이클 완결 + `p2a_proposals.mjs` queue/review/curation/patch draft MVP. 자동 적용은 금지 |
| staged proposal→review→curation→draft→approval→apply | ✅ | `skill-proposal.schema.json` + `proposal-review.schema.json` + `proposal-curation.schema.json` + `proposal-patch-draft.schema.json` + `p2a_proposals.mjs` + `p2a-skill-curator` |
...
- ✅ **Hermes proposal queue/review/curation/patch draft MVP** — 완료. `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap을 proposal JSON으로 축적하고 deterministic review/curation/patch-draft/digest/validate를 제공한다. 자동 적용은 계속 금지.
