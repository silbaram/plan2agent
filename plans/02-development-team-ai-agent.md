...
작성일: 2026-06-16 · 갱신일: 2026-06-24 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차와 오케스트레이션 runtime 1단계 완료, Hermes proposal queue/review/curation/patch draft/approval execution bridge 완료, PTY+Electron 감독 GUI MVP 완료, 실행기는 감독형 방향(구독 로그인 — 무인은 API 키 도입 시 보류)
...
| Supervised orchestration planner/runtime | MVP 1차 + runtime 1단계 완료 | `p2a_orchestrate.mjs`, `orchestration-plan.schema.json`, `orchestration-runtime.schema.json`, `p2a_execute --orchestration-plan`, `runs/<runId>.orchestration*.json` |
...
| Agent 자동 실행 | 부분 구현 | Phase 1 CLI semi-auto, supervised orchestration plan, GUI PTY 감독 실행 완료. team scheduler, retry, PR 연동은 후속 |
...
- **개발팀 컨셉 - Team Big Five**: 복잡한 task를 `team-lead`/`contributor`/`performance-monitor` 역할로 협업·교차검증한다. 현재 P2A에는 Team Big Five adapter 설치, `p2a-performance-monitor`, `p2a-dev-orchestrator` MVP 1차, deterministic solo/team triage, runtime shared mental model과 communication log가 들어와 있다. PTY 팀 실행기와 자동 role/monitor 호출은 후속이다.
...
- 여러 Codex/Claude/Gemini CLI 세션을 orchestration role별로 조율하는 team scheduler와 자동 role/monitor 호출.
...
| supervised orchestration plan/runtime MVP | 완료 | `p2a_orchestrate.mjs`, run/runtime sidecar, monitor gate fixture |
...
| LD-4 | 실행 모드 | `solo`/`solo_monitor`/`team` deterministic plan + runtime sidecar + monitor gate MVP. 실제 multi-session PTY scheduler는 후속 | `p2a_orchestrate.mjs`와 `p2a_execute --orchestration-plan`이 plan/runtime sidecar 계약을 제공한다 |
...
| supervised orchestration plan/runtime | `p2a_orchestrate.mjs`, `schemas/orchestration-plan.schema.json`, `schemas/orchestration-runtime.schema.json`, `p2a-dev-orchestrator`, run/runtime sidecar |
...
- **현재 기본 = supervised single task + 선택적 orchestration plan/runtime/monitor**: `p2a_execute.mjs`가 ready 확인, run start, manual launcher prompt, verify, finish, task done/block 전이를 묶고, `p2a_orchestrate.mjs`는 필요한 경우 역할별 실행 계획, runtime shared mental model, communication log, monitor gate를 sidecar로 연결한다. `p2a-dev-execution`은 사람이 감독하는 구현 절차를 안내한다.
...
- **팀 모드**: 복잡한 task의 team orchestration은 Plan2Agent-native runtime sidecar까지 기록한다. Team Big Five adapter 설치와 role handoff는 가능하지만, 여러 CLI 세션을 자동으로 열고 조율하는 scheduler는 아직 없다.
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 + runtime 1단계 완료)
   - 목적: Team Big Five의 team-lead 역할을 P2A-native로 분리한다.
   - 완료 범위: ready task 1건을 `solo`/`solo_monitor`/`team`으로 deterministic triage하고, 역할별 prompt, run/runtime sidecar, shared mental model, communication log, monitor gate를 남긴다.
   - 후속 범위: 실제 multi-session PTY scheduler, 자동 role/monitor 호출, retry/ask/blocked 판단 자동화.
...
5. **오케스트레이션 런타임 고도화 2단계** (다음 추천)
   - 목적: runtime sidecar를 기반으로 role별 session 상태, 자동 role/monitor 호출, retry/ask/blocked 판단으로 확장한다.
   - 선행 조건: 현재 deterministic plan/sidecar/monitor gate와 GUI start/finish lifecycle이 green이어야 한다.
...
- 🟡 **orchestrator / triage** — MVP 1차 + runtime 1단계 완료. `p2a_orchestrate.mjs`와 run/runtime sidecar가 정본이며, 실제 multi-session scheduler와 자동 role/monitor 호출은 후속.
- ✅ **PTY+Electron 감독 GUI** — MVP 완료. node-pty/xterm 기반 PTY session, start/finish lifecycle, task/run/artifact 표시, 한글/영문 UI가 들어왔다. orchestration role handoff와 team scheduler 표시는 후속.
