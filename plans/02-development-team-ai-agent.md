...
| Supervised orchestration planner/runtime | MVP 1차 + runtime 1단계 + supervised scheduler 2단계 + provider execution guide 완료 | `p2a_orchestrate.mjs`, `orchestration-plan.schema.json`, `orchestration-runtime.schema.json`, `next-role/role-prompt/mark-role`, roleProfile/override, executionGuide, blocked next action, `p2a_execute --orchestration-plan`, `runs/<runId>.orchestration*.json` |
...
| Agent 감독 실행 | 부분 구현 | Phase 1 CLI semi-auto, supervised orchestration plan, provider execution guide, GUI PTY 감독 실행 완료. provider-native team runner adapter, PR 연동은 후속 |
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 + runtime 1단계 + supervised scheduler 2단계 완료)
   ...
   - 완료 범위: ready task 1건을 `solo`/`solo_monitor`/`team`으로 deterministic triage하고, 역할별 prompt, roleProfile/override, provider capability, executionGuide, run/runtime sidecar, shared mental model, communication log, next-role/role-prompt/mark-role 감독형 scheduler, blocked next action, monitor gate를 남긴다.
   - 후속 범위: provider-native team runner adapter. 구독 로그인 기반 무인 실행과 자동 role/monitor 호출은 계속 제외한다.
...
5. **오케스트레이션 런타임 고도화 2단계** (✅ 감독형 scheduler + provider execution guide 완료)
   - 완료 범위: runtime sidecar를 기반으로 `next-role`, `role-prompt`, `mark-role`을 제공한다. 사람이 공식 CLI/앱을 열고 prompt를 붙여넣은 뒤 결과를 기록하는 흐름만 지원한다. provider capability matrix, roleProfile 자동 선택/override, executionGuide, blocked next action hint까지 plan/runtime/GUI에 기록한다.
   - 후속 범위: provider-native team runner adapter. Claude는 native agent teams/subagents, Codex는 skills/custom agents/명시 subagent prompt, Gemini는 planning/review/monitor read-only 보조로 둔다. API 기반 완전 자동 개발은 비용상 보류한다. 자동 role/monitor 호출과 무인 실행은 제외한다.
