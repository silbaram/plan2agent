...
작성일: 2026-06-16 · 갱신일: 2026-06-24 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차 완료, Hermes proposal queue MVP 완료, 실행기는 감독형 방향(구독 로그인 — 무인은 API 키 도입 시 보류)
...
| Skill evolution | proposal queue MVP 완료 | `p2a-skill-curator`, `schemas/skill-proposal.schema.json`, `p2a_proposals.mjs` |
...
- `p2a_proposals.mjs`: run log, orchestration sidecar, monitor verdict에서 proposal 후보를 생성하고 list/show/validate/digest를 제공한다.
...
- proposal queue를 curator가 자동 리뷰하고 승인 후보 diff 초안까지 만드는 흐름.
...
| Hermes proposal queue MVP | 완료 | `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap에서 proposal JSON을 생성·검증·요약 |
...
| LD-5 | 자가 발전 | JSON proposal queue + read-only curator + 사람 승인 | `skill-proposal.schema.json`, `p2a_proposals.mjs`, `p2a-skill-curator`가 존재한다 |
...
| retrospective proposal | `schemas/skill-proposal.schema.json`, `.agents/agents/p2a-skill-curator.md`, `p2a_proposals.mjs` |
...
| Hermes proposal queue | `p2a_proposals.mjs`, `validate_artifacts --proposals-dir`, scaffold/handoff manifest |
...
- `p2a_proposals.mjs`가 `failed`/`blocked` run, monitor verdict, verification gap을 읽어 proposal queue를 만든다.
...
| 자가 발전 (proposal→curator→승인→적용) | ✅ | 첫 사이클 완결 + `p2a_proposals.mjs` queue MVP. 자동 적용은 금지 |
| staged proposal→review→approval→apply | ✅ | `skill-proposal.schema.json` + `p2a_proposals.mjs` + `p2a-skill-curator` |
...
- ✅ **Hermes proposal queue MVP** — 완료. `p2a_proposals.mjs`가 run failure, monitor verdict, verification gap을 proposal JSON으로 축적하고 digest/validate를 제공한다. 자동 적용은 계속 금지.
