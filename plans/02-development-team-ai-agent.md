...
작성일: 2026-06-16 · 갱신일: 2026-06-25 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차와 오케스트레이션 runtime 1단계/감독형 scheduler 2단계/failure-policy 완료, Hermes proposal queue/review/curation/patch draft/approval execution bridge 완료, PTY+Electron 감독 GUI MVP 완료, provider-native team orchestration 후속 방향 확정, 실행기는 감독형 방향(구독 로그인 foreground 사용, API 기반 완전 자동은 비용상 보류)
...
자동 multi-session scheduler나 PR 생성기는 아직 없다. 대신 **Codex(native sandbox)·Claude(deterministic confinement) write-capable implementer**와, **CLI 공통으로 동작하는 독립 monitor·skill curator(둘 다 read-only)·retrospective proposal schema/queue/review/curation/patch draft/approval gate**, ready task 1건의 `plan/start/finish/status` lifecycle을 묶는 `p2a_execute.mjs` Phase 1 실행기, `solo`/`solo_monitor`/`team` 실행 계획과 `orchestration-runtime` sidecar 및 `next-role/role-prompt/mark-role/failure-policy` 감독형 scheduler를 제공하는 `p2a_orchestrate.mjs` MVP, run failure/monitor verdict/verification gap을 proposal로 남기고 deterministic review/curation/patch-draft/approval artifact와 maintenance task를 만드는 `p2a_proposals.mjs` MVP, approval artifact를 `p2a_execute --approval`로 감독형 실행에 연결하는 bridge, PTY session/start-finish lifecycle을 제공하는 `apps/p2a-gui` MVP까지 들어와 있다. Claude confinement은 scaffold가 설치하는 공통 deny-rules + cross-platform PreToolUse hook(symlink/junction 우회까지 canonicalize로 차단, fail-closed)에 macOS/Linux는 OS sandbox 토글을 더한 형태다. API 요금제 기반 완전 자동 개발은 비용상 보류하며, 구독 로그인 기반 Codex/Claude/Gemini 사용은 foreground 공식 CLI/앱을 사람이 승인·감독하는 방식으로 제한한다. 단, 그 foreground 세션 내부에서 provider가 공식 지원하는 skills/subagents/custom agents/agent teams/extensions를 사용하는 것은 허용되는 감독형 자동화다. Gemini implementer는 설계상 read-only로 고정한다(자율/write 미추진 결정, LD-8). GUI MVP 범위는 `plans/03-p2a-gui-mvp.md`에서 관리한다.

감독형 자동화 정의:

- 허용: 사용자가 공식 Codex/Claude/Gemini CLI/앱을 foreground로 열고, P2A가 출력한 role prompt를 붙여넣은 뒤, 그 세션 내부에서 provider-native skill/subagent/custom agent/agent team/extension이 백엔드·프론트·디자인·QA 역할을 수행한다.
- 허용: P2A가 어떤 role을 어떤 provider-native 기능에 위임할지 prompt와 guide로 명시하고, 결과를 사람이 확인한 뒤 `record`/`mark-role`/`finish`로 기록한다.
- 제외: P2A가 SDK/API로 provider를 직접 호출하거나, provider CLI/앱을 background/headless로 실행하거나, 여러 세션을 무인 loop로 돌리는 방식.
- 제외: 세션 쿠키·토큰 재사용, 계정 로테이션, rate limit 우회, browser/background loop.
...
| Supervised orchestration planner/runtime | MVP 1차 + runtime 1단계 + supervised scheduler 2단계 + failure-policy + provider execution guide 완료 | `p2a_orchestrate.mjs`, `orchestration-plan.schema.json`, `orchestration-runtime.schema.json`, `next-role/role-prompt/mark-role/failure-policy`, roleProfile/override, executionGuide, blocked next action, `p2a_execute --orchestration-plan`, `runs/<runId>.orchestration*.json` |
...
| Agent 감독 실행 | 완료 | Phase 1 CLI semi-auto, supervised orchestration plan/runtime/scheduler, provider execution guide/runner doctor/capability evidence, GUI PTY 감독 실행 완료. PR 연동은 후속 |
...
- **개발팀 컨셉 - Team Big Five**: 복잡한 task를 `team-lead`/`contributor`/`performance-monitor` 역할로 협업·교차검증한다. 현재 P2A에는 Team Big Five adapter 설치, `p2a-performance-monitor`, `p2a-dev-orchestrator` MVP 1차, deterministic solo/team triage, runtime shared mental model, communication log, supervised role scheduler, provider capability matrix, provider-native runner guide/doctor, capability evidence 표시가 들어와 있다. 기본은 single-provider team이며 Claude는 native agent teams/subagents, Codex는 skills/custom agents/명시 subagent prompt, Gemini는 planning/review/monitor read-only 보조로 둔다. provider-native 기능은 사용자가 연 foreground 세션 안에서 쓰도록 prompt에 위임하고, P2A가 직접 role/monitor 세션을 실행하는 것은 비목표다.
...
- `p2a_orchestrate next-role/role-prompt/mark-role/failure-policy`: runtime 상태에서 다음 role을 계산하고, 사람이 공식 CLI/앱에 붙여넣을 prompt를 출력하고, 그 prompt 안에서 provider-native skill/subagent/custom agent/agent team 사용을 명시하며, 사람이 수행한 role 상태 전이를 기록한다. blocked/failed runtime의 다음 조치도 `retry`/`ask_user`/`stop` 중 하나로 계산한다. 이 명령들은 Codex/Claude/Gemini 프로세스를 자동 실행하지 않는다.
...
- P2A가 provider CLI/API를 직접 실행해 monitor/curator를 자동 호출하고 결과를 강제하는 scheduler. foreground 세션 내부의 provider-native subagent/skill 사용은 허용한다.
...
| supervised orchestration plan/runtime/scheduler MVP | 완료 | `p2a_orchestrate.mjs`, run/runtime sidecar, next-role/role-prompt/mark-role/failure-policy, monitor gate fixture |
...
| provider-native team runner adapter | 완료 | capability matrix, role profile/override, executionGuide, runner-guide, runner-doctor, provider capability evidence 표시 완료. 계정 내부 기능 자동 introspection은 비목표/후속 검토 |
...
| LD-4 | 실행 모드 | `solo`/`solo_monitor`/`team` deterministic plan + runtime sidecar + supervised scheduler + failure-policy + monitor gate + provider-native runner guide/doctor | `p2a_orchestrate.mjs`와 `p2a_execute --orchestration-plan`이 plan/runtime/scheduler 계약을 제공하고, provider capability matrix, executionGuide, capability evidence doctor, retry/ask_user/stop 판단까지 붙었다 |
...
| 항목 | 미루는 이유 |
| --- | --- |
| 일반 multi-provider PTY session coordinator | provider-native team runner는 완료. 여러 회사를 동시에 섞어 터미널을 조율하는 방식은 운영 부담이 커서 후순위로 둔다 |
...
| 병렬 scheduler | 자동 조율 대신 provider-native foreground delegation으로 제한한다. 사람이 공식 CLI/앱 세션을 열고 P2A는 role/prompt/order/state만 조율한다 |
...
- **팀 모드**: 복잡한 task의 team orchestration은 Plan2Agent-native runtime sidecar와 supervised scheduler까지 기록한다. Team Big Five adapter 설치와 role handoff, provider capability matrix, provider-native runner guide/doctor, capability evidence 표시는 완료됐다. 여러 provider 구현 agent를 동시에 섞는 방식을 기본값으로 두지 않는다. Claude는 native agent teams/subagents, Codex는 skills/custom agents/명시 subagent prompt, Gemini는 planning/review/monitor read-only 보조로 둔다. 이 provider-native 기능은 사용자가 연 foreground 세션 안에서 동작하며, P2A는 직접 실행하지 않는다.
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 + runtime 1단계 + supervised scheduler 2단계 + failure-policy + provider-native runner guide/doctor 완료)
   - 목적: Team Big Five의 team-lead 역할을 P2A-native로 분리한다.
   - 완료 범위: ready task 1건을 `solo`/`solo_monitor`/`team`으로 deterministic triage하고, 역할별 prompt, roleProfile/override, provider capability, executionGuide, runner guide/doctor, capability evidence, run/runtime sidecar, shared mental model, communication log, next-role/role-prompt/mark-role 감독형 scheduler, failure-policy, blocked next action, monitor gate를 남긴다.
   - 후속 범위: agent-generated orchestration plan. 구독 로그인 기반 무인 실행, 계정 내부 자동 introspection, P2A가 직접 실행하는 role/monitor 자동 호출은 계속 제외한다.
...
5. **오케스트레이션 런타임 고도화 2단계** (✅ 감독형 scheduler + failure-policy + provider execution guide/doctor 완료)
   - 완료 범위: runtime sidecar를 기반으로 `next-role`, `role-prompt`, `mark-role`, `failure-policy`를 제공한다. 사람이 공식 CLI/앱을 열고 prompt를 붙여넣은 뒤 결과를 기록하는 흐름만 지원한다. provider capability matrix, roleProfile 자동 선택/override, executionGuide, runner guide/doctor, capability evidence, blocked next action hint, retry/ask_user/stop 판단까지 plan/runtime/GUI에 기록한다.
   - 후속 범위: agent-generated orchestration plan. Claude는 native agent teams/subagents, Codex는 skills/custom agents/명시 subagent prompt, Gemini는 planning/review/monitor read-only 보조로 둔다. API 기반 완전 자동 개발은 비용상 보류한다. 자동 role/monitor 호출과 무인 실행, 계정 내부 자동 introspection은 제외한다.
...
| team-lead / orchestrator | ✅ | `p2a-dev-orchestrator` MVP 1차 + runtime 1단계 + supervised scheduler 2단계 + failure-policy + provider-native runner guide/doctor/capability evidence 완료 |
...
| supervised role scheduler | ✅ | `next-role`, `role-prompt`, `mark-role`, `failure-policy`. 프로세스 자동 실행 없음 |
...
- ✅ **orchestrator / triage** — MVP 1차 + runtime 1단계 + supervised scheduler 2단계 + failure-policy + provider-native runner guide/doctor/capability evidence 완료. `p2a_orchestrate.mjs`와 run/runtime sidecar가 정본이며, 후속은 agent-generated orchestration plan이다. 자동 role/monitor 호출과 계정 내부 자동 introspection은 제외한다.
...
- 이번 단계의 provider-native team orchestration adapter는 완료됐다. 다음 추천 구현은 **agent-generated orchestration plan** 또는 **PR 생성/리뷰 상태 연동**이다.
