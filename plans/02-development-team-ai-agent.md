...
작성일: 2026-06-16 · 갱신일: 2026-06-25 · 상태: Level 1 완료, Codex 실행 계층 부분 구현, 자가 발전 첫 사이클 적용, Claude 자율 confinement 구현(deny+hook+macOS sandbox, Level 2 step 2), 실패 분류(C-①) 완료, 감독형 단일 task 실행기 Phase 1 완료, `p2a-dev-orchestrator` MVP 1차와 오케스트레이션 runtime 1단계/감독형 scheduler 2단계 완료, Hermes proposal queue/review/curation/patch draft/approval execution bridge 완료, PTY+Electron 감독 GUI MVP 완료, 실행기는 감독형 방향(구독 로그인 foreground 사용, API 기반 완전 자동은 비용상 보류)
...
| Agent 감독 실행 | 부분 구현 | Phase 1 CLI semi-auto, supervised orchestration plan, GUI PTY 감독 실행 완료. 감독형 multi-session coordinator, retry 보조, PR 연동은 후속 |
...
- **개발팀 컨셉 - Team Big Five**: 복잡한 task를 `team-lead`/`contributor`/`performance-monitor` 역할로 협업·교차검증한다. 현재 P2A에는 Team Big Five adapter 설치, `p2a-performance-monitor`, `p2a-dev-orchestrator` MVP 1차, deterministic solo/team triage, runtime shared mental model, communication log, supervised role scheduler가 들어와 있다. 후속은 자동 role/monitor 호출이 아니라, 사용자가 foreground 공식 CLI/앱 세션을 열고 p2a가 role/prompt/order/state를 조율하는 감독형 multi-session PTY coordinator다.
...
- 여러 Codex/Claude/Gemini CLI 세션을 무인으로 열고 조율하는 PTY scheduler와 자동 role/monitor 호출.
...
| LD-4 | 실행 모드 | `solo`/`solo_monitor`/`team` deterministic plan + runtime sidecar + supervised scheduler + monitor gate MVP. 후속은 감독형 multi-session PTY coordinator | `p2a_orchestrate.mjs`와 `p2a_execute --orchestration-plan`이 plan/runtime/scheduler 계약을 제공한다 |
...
| 병렬 scheduler | 자동 조율 대신 감독형 multi-session PTY coordinator로 제한한다. 사람이 공식 CLI/앱 세션을 열고 p2a는 role/prompt/order/state만 조율한다 |
...
- **팀 모드**: 복잡한 task의 team orchestration은 Plan2Agent-native runtime sidecar와 supervised scheduler까지 기록한다. Team Big Five adapter 설치와 role handoff는 가능하지만, 여러 CLI 세션을 무인으로 열고 조율하는 scheduler는 만들지 않는다. 후속은 사람이 foreground 세션을 열고 p2a가 role/prompt/order/state만 조율하는 감독형 coordinator다.
...
3. **`p2a-dev-orchestrator` MVP** (✅ 1차 + runtime 1단계 + supervised scheduler 2단계 완료)
   ...
   - 후속 범위: 감독형 multi-session PTY coordinator, retry/ask/blocked 판단 보조. 구독 로그인 기반 무인 실행과 자동 role/monitor 호출은 계속 제외한다.
...
5. **오케스트레이션 런타임 고도화 2단계** (✅ 감독형 scheduler 완료)
   ...
   - 후속 범위: GUI에서 scheduler 상태를 표시하고, API 기반 완전 자동 개발은 비용상 보류한다. 자동 role/monitor 호출과 무인 실행은 제외한다.
...
| team-lead / orchestrator | 🟡 | `p2a-dev-orchestrator` MVP 1차 + runtime 1단계 + supervised scheduler 2단계 완료. 후속은 감독형 multi-session coordinator |
...
- 🟡 **orchestrator / triage** — MVP 1차 + runtime 1단계 + supervised scheduler 2단계 완료. `p2a_orchestrate.mjs`와 run/runtime sidecar가 정본이며, 후속은 감독형 multi-session PTY coordinator다. 자동 role/monitor 호출은 제외한다.
- ✅ **PTY+Electron 감독 GUI** — MVP 완료. node-pty/xterm 기반 PTY session, start/finish lifecycle, task/run/artifact 표시, orchestration runtime/scheduler 표시, role prompt 복사, 수동 `mark-role`, 한글/영문 UI가 들어왔다. 후속은 감독형 multi-session coordinator다.
