...
자동 PTY 실행기나 PR 생성기는 아직 없다. 대신 Codex에서 사용할 수 있는 write-capable implementer 계약, 독립 monitor 계약, retrospective proposal schema, skill curator 계약까지는 들어와 있다.
...
| Claude/Gemini implementer | 계약만 존재 | mirror는 생성되지만 write tool은 부여하지 않는다. read/search 수준 |
...
- Claude/Gemini write-capable implementer 실증.
...
| LD-3 | 대상 CLI | Codex 우선. Claude/Gemini는 read/search mirror만 유지 | Claude/Gemini write tool 권한 모델 검증은 후속 |
...
| Claude/Gemini write-capable implementer | CLI별 write permission/sandbox 모델 검증 필요 |
...
| Execution | ready task를 실제 코드 변경으로 수행 | Codex workspace-write, Claude/Gemini read-only mirror | `p2a-dev-execution`, `p2a-implementer` |
