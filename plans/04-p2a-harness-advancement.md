...
| capability 확장 | `enhance dev-skills`, `enhance memory`, `enhance orchestration`, `enhance proposals` 흐름을 capability 단위로 분리했다. |
...
이렇게 나누면 작은 프로젝트는 planning/task/run만 쓰고, 필요한 시점에 Memory, orchestration을 붙일 수 있다.
...
| `run` footer가 copy-paste 가능한 resume command를 출력 | `p2a_runs start/finish` footer에 `resume`, `status`, `finish`, `review` 명령을 출력 | P2 |
...
| observability를 trace, prompt-response log, analytics, third-party tier로 구분 | P2A Memory에 trace/content/analytics/search tier를 둠 | P2 |
...
| `validate_artifacts.mjs`, `check_cli_parity.mjs` | 사용자가 먼저 실행할 수 있는 `info/doctor` 표면 | 개별 검증 스크립트를 `p2a doctor` 아래에 묶고 결과 JSON을 재사용 |
...
| status/diff/push/pull/conflict UX | P2A CLI |
| embedding 생성 | 외부 클라이언트 또는 별도 worker |

Memory 서버가 agent를 실행하거나 하네스 판단을 대신하지 않는 현재 경계는 유지해야 한다. 대신 P2A CLI가 git client처럼 동기화 client 역할을 한다.
...
이 구조가 잡히면 P2A CLI는 프로젝트 기억을 검색하고 다음 행동을 추천하는 운영 표면으로 확장될 수 있다.
...
4. `info/doctor`가 이 명령 결과를 읽도록 한다.
...
2. `p2a enhance memory/orchestration/proposals`를 capability 단위로 분리해 prototype-first 확장 흐름을 만든다.
...
5. run footer에 copy-paste 가능한 resume/status/finish/review 명령을 남긴다.
