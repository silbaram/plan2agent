...
- For `kind: approval` with `options`, present every structured option and wait for the user's explicit choice. For `review`, keep the iteration open and review the completed implementation. If a finding requires code changes, replace the placeholders in the returned `action.remediation` command with the owning completed task id and a concrete finding note, run it, then run `next` to enter a new remediation run through the normal lifecycle. A clean review runs `next` again and returns to the same decision. For `close`, the user's exact choice authorizes only the returned nested close command; execute it without treating a prior clean review as approval.
